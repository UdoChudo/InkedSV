...
local ItemBar = _G["InkedSV_QuestItemBar"];
ItemBar.Buttons = {};
...
do
    ...
    CreateQuestItemButton = function(index)
    	local buttonName = "InkedSV_QuestButton" .. index
...
local GetQuestRow = function(self, index)
	if(not self.Rows[index]) then
		...
		row.Header.Level:SetFontObject(InkedSV_Font_Quest_Number);
		...
		row.Header.Text:SetFontObject(InkedSV_Font_Quest);
		...
		row.Header.Zone:SetFontObject(InkedSV_Font_Quest);
		...
		row.Timer.TimeLeft:SetFontObject(InkedSV_Font_Quest_Number);
...
function MOD:InitializeQuests()
	...
	quests.Header.Text:SetFontObject(InkedSV_Font_Quest_Header);
	...
	quests.Header.Divider:SetTexture([[Interface\AddOns\InkedSV\assets\textures\DROPDOWN-DIVIDER]]);
