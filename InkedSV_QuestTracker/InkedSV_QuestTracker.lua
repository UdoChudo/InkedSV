...
local OBJECTIVE_GetStatus = function(self)
	if(not self.Status) then
		...
		status.Label:SetFontObject(InkedSV_Font_Quest_Number)
		status.Label:SetTextColor(1,1,1)
		status.Label:SetText('')
...
local OBJECTIVE_HEADER_Get = function(self, index)
	if(not self.Rows[index]) then
		...
		objective.Text:SetFontObject(InkedSV_Font_Quest);
...
function MOD:Load()
	self.Headers = {}
	self.Docklet = SV.Dock:NewDocklet("BottomRight", "InkedSV_QuestTracker", "Quest Tracker", MOD.media.dockIcon);

	self:InitializePopups()
	self:InitializeActive()

	local scrollFrame = CreateFrame("ScrollFrame", "InkedSV_QuestTrackerScrollFrame", self.Docklet);
	...
	local scrollBar = CreateFrame("Slider", "InkedSV_QuestTrackerScrollFrameScrollBar", scrollFrame);
	...
	scrollBar:SetScript("OnValueChanged", function(self, argValue)
		_G.InkedSV_QuestTrackerScrollFrame:SetVerticalScroll(argValue)
	end)
	SV.API:Set("ScrollBar", scrollBar)

	local scrollChild = CreateFrame("Frame", "InkedSV_QuestTrackerScrollFrameScrollChild", scrollFrame);
