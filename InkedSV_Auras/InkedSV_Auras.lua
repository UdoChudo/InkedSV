...
MOD.Holder = CreateFrame("Frame", "InkedSV_AurasAnchor", UIParent)
MOD.HyperBuffFrame = CreateFrame('Frame', 'InkedSV_ConsolidatedBuffs', UIParent)
--[[
##########################################################
LOCAL VARS
##########################################################
...
do
	...
	function MOD:CreateIcon(aura, has_attrib)
		...
		aura.texture:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
		...
		aura.count:SetFontObject(InkedSV_Font_Aura)
		...
		aura.time:SetFontObject(InkedSV_Font_Aura)
...
function MOD:UpdateAuraHeader(auraHeader, auraType)
	...
	if(auraType == "buffs") then
		...
		auraHeader:SetAttribute("weaponTemplate", ("InkedSV_AuraTemplate%d"):format(db.size))
	end
	...
	auraHeader:SetAttribute("template", ("InkedSV_AuraTemplate%d"):format(db.size))
...
function MOD:ReLoad()
	...
	local CB_HEIGHT = Minimap:GetHeight() - 50;
	local CB_WIDTH = (CB_HEIGHT / maxShown) + 4;
	if(InkedSV_MinimapFrame) then
		CB_HEIGHT = InkedSV_MinimapFrame:GetHeight() - 50;
	end
	self.Holder:SetSize(CB_WIDTH, CB_HEIGHT)
	AURA_FADE_TIME = SV.db.Auras.fadeBy
	self:UpdateAuraHeader(InkedSV_PlayerBuffs, "buffs");
	self:UpdateAuraHeader(InkedSV_PlayerDebuffs, "debuffs");
...
function MOD:Load()
	...
	if(SV.db.Auras.aurasEnabled) then
		...
		local buffHeader = CreateFrame("Frame", "InkedSV_PlayerBuffs", self.Holder, "SecureAuraHeaderTemplate")
		...
		local debuffHeader = CreateFrame("Frame", "InkedSV_PlayerDebuffs", self.Holder, "SecureAuraHeaderTemplate")
