...
local ProcWatch = CreateFrame('Frame', 'InkedSV_ProcWatch', UIParent);
...
local function CreateProcIcon()
	...
	proc.icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
	...
	proc.count:SetFontObject(InkedSV_Font_Aura)
	...
	proc.time:SetFontObject(InkedSV_Font_Aura)
...
function MOD:InitializeProcWatch()
	...
	local ProcsAnchor = CreateFrame('Frame', 'InkedSV_ProcWatchFrame', SV.Screen)
