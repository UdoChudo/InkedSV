--- InkedSV is our global addon object.
-- Inked Shadow Vaults Core Module.
-- @module InkedSV
...
--    local global = "InkedSV_Global"    -- reference to SavedVariables
--    local errors = "InkedSV_Errors"    -- reference to SavedVariables
--    local private = "InkedSV_Private"  -- reference to SavedVariables
--    local media = "InkedSV_Media"      -- reference to SavedVariables
--    local shared = "InkedSV_Shared"    -- reference to SavedVariables
--
...
--    local ISV = Registry:NewCore(global, errors, private, media, shared)
--
--    -- METHOD 2 ----------------------------------------------------------------
...
_G.BINDING_HEADER_InkedSV = "Inked Shadow Vaults";
_G.BINDING_NAME_InkedSV_MARKERS = "Raid Markers";
_G.BINDING_NAME_InkedSV_DOCKS = "Toggle Both Docks";
_G.BINDING_NAME_InkedSV_DOCKS_LEFT = "Toggle Left Dock";
_G.BINDING_NAME_InkedSV_DOCKS_RIGHT = "Toggle Right Dock";
_G.BINDING_NAME_InkedSV_RIDE = "Let's Ride";
_G.BINDING_NAME_InkedSV_DRAENORZONE = "Draenor Zone Ability";
_G.BINDING_NAME_InkedSV_FRAMEDEBUGGER = "Inked Shadow Vaults: Frame Analyzer";

_G.SlashCmdList.RELOADUI = ReloadUI
_G.SLASH_RELOADUI1 = "/rl"
_G.SLASH_RELOADUI2 = "/reloadui"

_G.InkedSV_ICON_COORDS = {0.1, 0.9, 0.1, 0.9};
...
---- Global InkedSV Object. We have to send the names ----
---- of our three SavedVariables files since the ----
---- WoW API has no method for parsing them in LUA. ----

local SV = InkedSVLib:NewCore("InkedSV_Global", "InkedSV_Errors", "InkedSV_Private", "InkedSV_Media", "InkedSV_Shared")

SV.ConfigID           = "InkedSV_Options";
...
SV.Screen = CreateFrame("Frame", "InkedSVParent", UIParent);
...
InkedSVLib.CONSTRAINTS.IGNORED["LAYOUT"] = true;
InkedSVLib.CONSTRAINTS.IGNORED["REPORT_SLOTS"] = true;

InkedSVLib.CONSTRAINTS.PROTECTED["extended"] = true;
InkedSVLib.CONSTRAINTS.PROTECTED["shared"] = true;
InkedSVLib.CONSTRAINTS.PROTECTED["color"] = true;
InkedSVLib.CONSTRAINTS.PROTECTED["bordercolor"] = true;
InkedSVLib.CONSTRAINTS.PROTECTED["Filters"] = true;
...
SV.defaults           = {
    ...
    REPORT_SLOTS = {
        ['1'] = { "None", "None", "None" },
        ['2'] = { "None", "None", "None" },
        ['3'] = { "Experience Bar", "Time", "System" },
        ['4'] = { "Gold", "Friends", "Durability Bar" },
    },
    Reports = {
        backdrop = false,
        shortGold = true,
...
SV.Options = {
    ...
    args = {
        InkedSV_Header = {
            order = 1,
            type = "header",
            name = ("Powered By |cffff9900InkedSV|r - %s: |cff99ff33%s|r"):format(SV.L["Version"], SV.Version),
            width = "full"
        },
        profiles = {
            order = 9997,
...
local function _removedeprecated()
    ---- BEGIN DEPRECATED ----
    if(_G.InkedSV_Filters and (not _G.InkedSV_TRANSFER_WIZARD)) then
        _tablecopy(SV.db.Filters, _G.InkedSV_Filters)
        _G.InkedSV_Filters = nil
    end
    ---- END DEPRECATED ----
end
...
---------------------------------------------------------------------
-- Utilities
-- @section UTILITIES Utilities used and shared by the InkedSV core.
---------------------------------------------------------------------
...
---------------------------------------------------------------------
-- Reset all InkedSV created settings to defaults.
-- @function ResetAllUI
---------------------------------------------------------------------
...
function SV:VersionCheck()
    local delayed;
    if(_G.InkedSV_TRANSFER_WIZARD) then
        local copied = InkedSVLib:GetSafeData("transfer_wizard_used");
        if(not copied) then
            delayed = true;
            _G.InkedSV_TRANSFER_WIZARD()
        end
    end
    if(not delayed) then
		--local version = SV.Version;
       local version = InkedSVLib:GetSafeData("install_version");
	   -- print(InkedSVLib:GetSafeData(InkedSV_Global.profiles.playerName))
		
		--if InkedSVLib:GetSafeData("install_version", SV.Version) == SV.Version then return end
		if InkedSVLib:GetSafeData(InkedSV_Global.profiles.playerName) == nil then
		--if InkedSVLib:GetSafeData("profileKeys") == nil then
		--playerName
       -- if(not version) then
