...
local function ScreenBorderVisibility()
	if SV.db.Dock.bottomPanel then
		InkedSV_DockBarBottom:Show()
	else
		InkedSV_DockBarBottom:Hide()
	end

	if SV.db.Dock.topPanel then
		InkedSV_DockBarTop:Show()
	else
		InkedSV_DockBarTop:Hide()
	end
end
...
local DockBar_CreateButton = function(self, displayName, globalName, texture, tipFunction, primaryTemplate, frameLink)
	local dockIcon = texture or [[Interface\AddOns\InkedSV\assets\textures\Dock\DOCK-ICON-ADDON]];
	local size = self.ToolBar:GetHeight();
	local template = "InkedSV_DockletButtonTemplate"
	local isAction = false;

	if(primaryTemplate) then
		template = primaryTemplate .. ", InkedSV_DockletButtonTemplate";
		isAction = true;
	end
...
MOD.Top = _G["InkedSV_DockBarTop"];
--MOD.Top:SetParent(SV.Screen)
MOD.Bottom = _G["InkedSV_DockBarBottom"];
MOD.TopCenter = _G["InkedSV_DockTopCenter"];
MOD.BottomCenter = _G["InkedSV_DockBottomCenter"];
...
for location, settings in pairs(DOCK_LOCATIONS) do
	MOD[location] = _G["InkedSV_Dock" .. location];
	MOD[location].Bar = _G["InkedSV_DockBar" .. location];
