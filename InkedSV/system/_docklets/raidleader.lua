...
local ToolButton_OnEnter = function(self, ...)
	InkedSV_RaidToolDockButton:SetPanelColor("highlight")
	InkedSV_RaidToolDockButton.Icon:SetGradient(unpack(SV.media.gradient.highlight))
...
local ToolButton_OnLeave = function(self, ...)
	InkedSV_RaidToolDockButton:SetPanelColor("default")
	InkedSV_RaidToolDockButton.Icon:SetGradient(unpack(SV.media.gradient.icon))
...
function MOD:LoadRaidLeaderTools()
	...
	self.RaidTool = SV.Dock:SetDockButton("TopLeft", RAID_CONTROL, "InkedSV_RaidToolDockButton", SV.media.dock.raidToolIcon);

	local dock = self.RaidTool.Parent

	self.RaidTool.Menu = CreateFrame("Frame", "InkedSV_RaidToolMenu", self.RaidTool, "SecureHandlerBaseTemplate");
	...
	local InkedSV_RaidToolToggle = CreateFrame("Button", "InkedSV_RaidToolToggle", self.RaidTool, "SecureHandlerClickTemplate")
	InkedSV_RaidToolToggle:SetAllPoints(self.RaidTool)
	InkedSV_RaidToolToggle:RemoveTextures()
	InkedSV_RaidToolToggle:SetNormalTexture("")
	InkedSV_RaidToolToggle:SetPushedTexture("")
	InkedSV_RaidToolToggle:SetHighlightTexture("")
	InkedSV_RaidToolToggle:SetFrameRef("InkedSV_RaidToolMenu", InkedSV_RaidToolMenu)
	InkedSV_RaidToolToggle:SetAttribute("_onclick", [=[
		local raidUtil = self:GetFrameRef("InkedSV_RaidToolMenu");
		local closeButton = self:GetFrameRef("InkedSV_RaidToolCloseButton");
		raidUtil:Show();
	...
	InkedSV_RaidToolToggle:SetScript("PostClick", function(self) self:RemoveTextures(); InkedSV_RaidToolMenu.toggled = true end);
	InkedSV_RaidToolToggle:HookScript("OnEnter", ToolButton_OnEnter)
	InkedSV_RaidToolToggle:HookScript("OnLeave", ToolButton_OnLeave)
	SV:ManageVisibility(self.RaidTool);

	--Close Button
	local close = NewToolButton("InkedSV_RaidToolCloseButton", self.RaidTool.Menu, "UIMenuButtonStretchTemplate, SecureHandlerClickTemplate", 30, 18, "BOTTOM", self.RaidTool.Menu, "BOTTOM", 0, 2, "X");
	close:SetAttribute("_onclick", [=[ self:GetParent():Hide(); ]=]);
	InkedSV_RaidToolToggle:SetFrameRef("InkedSV_RaidToolCloseButton", close)
	close:SetScript("PostClick", function() InkedSV_RaidToolMenu.toggled = false end);

	local disband = NewToolButton("InkedSV_RaidToolDisbandButton", self.RaidTool.Menu, "UIMenuButtonStretchTemplate", 109, 18, "TOP", self.RaidTool.Menu, "TOP", 0, -5, L['Disband Group'])
	...
	local rolecheck = NewToolButton("InkedSV_RaidToolRoleButton", self.RaidTool.Menu, "UIMenuButtonStretchTemplate", 109, 18, "TOP", disband, "BOTTOM", 0, -5, ROLE_POLL)
	...
	local ready = NewToolButton("InkedSV_RaidToolReadyButton", self.RaidTool.Menu, "UIMenuButtonStretchTemplate", 109, 18, "TOP", rolecheck, "BOTTOM", 0, -5, READY_CHECK)
	...
	local control = NewToolButton("InkedSV_RaidToolControlButton", self.RaidTool.Menu, "UIMenuButtonStretchTemplate", 109, 18, "TOP", ready, "BOTTOM", 0, -5, L['Raid Menu'])
