...
local function UpdateHearthOptions()
	...
	if(leftClick and type(leftClick) == "string") then
		InkedSV_Hearth:SetAttribute("tipText", leftClick)
		InkedSV_Hearth:SetAttribute("macrotext1", "/use [nomod]" .. leftClick)
	end

	local rightClick = GetHearthOption(HEARTH_RIGHT_CLICK);
	if(rightClick and type(rightClick) == "string") then
		InkedSV_Hearth:SetAttribute("tipExtraText", rightClick)
		InkedSV_Hearth:SetAttribute("macrotext2", "/use [nomod]" .. rightClick)
	end
end
...
local function LoadMiscTools()
	...
	if(SV.db.Dock.dockTools.hearth and (not InkedSV_Hearth)) then
		if(HEARTH_HEADER and type(HEARTH_HEADER) == "string") then
			local hearth = SV.Dock:SetDockButton("BottomLeft", HEARTH_HEADER, "InkedSV_Hearth", SV.media.dock.hearthIcon, Hearth_OnEnter, "SecureActionButtonTemplate")
	...
	-- SPEC BUTTON
	if(SV.db.Dock.dockTools.specswap and (not InkedSV_SpecSwap)) then

		local numSpecs = GetNumSpecializations()
		if(numSpecs and numSpecs > 1) then
			local specSwap = SV.Dock:SetDockButton("BottomLeft", L["Spec Swap"], "InkedSV_SpecSwap", SV.media.dock.specSwapIcon, SpecSwap_OnEnter)
			specSwap:SetClickCallbacks(SpecSwap_OnLeftClick, SpecSwap_OnRightClick);
		end
	end

	-- POWER BUTTON
	if(SV.db.Dock.dockTools.power and (not InkedSV_PowerButton)) then
		local power = SV.Dock:SetDockButton("BottomLeft", L["Power Button"], "InkedSV_PowerButton", SV.media.dock.powerIcon, PowerButton_OnEnter)
