...
SV.Dropdown = _G["InkedSV_DropdownFrame"];
...
function SV.Dropdown:Open(target, list, titleText, colWidth)
	...
	for i=1, #list do
		if(not self.option[i]) then
			...
			self.option[i].divider:SetTexture([[Interface\AddOns\InkedSV\assets\textures\DROPDOWN-DIVIDER]]);
			self.option[i].divider:Hide();
			...
			self.option[i].button.hoverTex:SetTexture([[Interface\AddOns\InkedSV\assets\textures\TITLE-HIGHLIGHT]]);
			...
			--InkedSV_DDSliderTemplate
			-- SLIDER
			self.option[i].slider = CreateFrame("Slider", nil, self.option[i], "InkedSV_TinySliderTemplate");
