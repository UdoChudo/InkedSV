...
local InkedSV_AlertFrame = CreateFrame("Frame", "InkedSV_AlertFrame", UIParent);
InkedSV_AlertFrame:SetPoint("TOP", InkedSV_DockTopCenter, "BOTTOM", 0, -115);
InkedSV_AlertFrame:SetSize(180, 20);
--[[
##########################################################
DEFINITIONS
##########################################################
...
SV.SystemAlert["CLIENT_UPDATE_REQUEST"] = {
	text = L["Detected that your InkedSV Config addon is out of date. Update as soon as possible."],
	button1 = OKAY,
	OnAccept = SV.fubar,
	state1 = 1
};
...
SV.SystemAlert["CHANGED_MANAGED_UISCALE"] = {
	text = L["You have changed your UIScale, because you have enabled InkedSV managed scaling, we will have to reload the UI to properly align everything."],
	button1 = OKAY,
...
SV.SystemAlert["TAINT_RL"] = {
	text = L["InkedSV has lost it's damned mind! I need to reload your UI to fix it."],
	button1 = ACCEPT,
	button2 = CANCEL,
	OnAccept = function()ReloadUI()end,
	timeout = 0,
...
local SysPop_Find = function(which, data)
	...
	for index = 1, MAX_STATIC_POPUPS, 1 do
		local frame = _G["InkedSV_SystemAlert"..index];
...
function SV:StaticPopup_Show(which, text_arg1, text_arg2, data)
	...
	if ( not dialog ) then
		...
		for i = index, MAX_STATIC_POPUPS do
			local frame = _G["InkedSV_SystemAlert"..i];
...
local AlertFramePostMove_Hook = function(forced)
	local b, c = InkedSV_AlertFrame_MOVE:GetCenter()
	local d = SV.Screen:GetTop()
	if(c > (d * 0.6)) then
		POSITION = "TOP"
		ANCHOR_POINT = "BOTTOM"
		YOFFSET = -10;
		InkedSV_AlertFrame_MOVE:SetText(InkedSV_AlertFrame_MOVE.textString.." (Grow Down)")
	else
		POSITION = "BOTTOM"
		ANCHOR_POINT = "TOP"
		YOFFSET = 10;
		InkedSV_AlertFrame_MOVE:SetText(InkedSV_AlertFrame_MOVE.textString.." (Grow Up)")
	end

	if(SV.RollFrames and SV.RollFrames[1]) then
		local lastFrame = InkedSV_AlertFrame;
		local newAnchor;
		...
		if(newAnchor) then
			...
			AlertFrame:SetPoint(POSITION, InkedSV_AlertFrame, POSITION, 0, 0)
		end
	else
		AlertFrame:ClearAllPoints()
		AlertFrame:SetPoint(POSITION, InkedSV_AlertFrame, POSITION, 0, 0)
...
function SV:StaticPopup_Hide(which, data)
	for index = 1, MAX_STATIC_POPUPS, 1 do
		local dialog = _G["InkedSV_SystemAlert"..index];
...
function SV:SavedPopup()
	if not _G["InkedSV_ConfigAlert"] then return end
	local alert = _G["InkedSV_ConfigAlert"]
...
local function LoadSystemAlerts()
	if not _G["InkedSV_ConfigAlert"] then
		local configAlert = CreateFrame("Frame", "InkedSV_ConfigAlert", UIParent)
		...
		bgtex:SetAllPoints()
		bgtex:SetTexture([[Interface\AddOns\InkedSV\assets\textures\Alert\SAVED-BG]])
		...
		fgtex:SetAllPoints()
		fgtex:SetTexture([[Interface\AddOns\InkedSV\assets\textures\Alert\SAVED-FG]])
		SetConfigAlertAnim(configAlert.fg)

		SV.Animate:Orbit(configAlert.bg, 10, false, true)
	end
	for i = 1, 4 do
		local alert = CreateFrame("Frame", "InkedSV_SystemAlert"..i, UIParent, "StaticPopupTemplate")
		alert:SetID(i)
		...
		alert.input = _G["InkedSV_SystemAlert"..i.."EditBox"];
		...
		alert.gold = _G["InkedSV_SystemAlert"..i.."MoneyInputFrameGold"];
		alert.silver = _G["InkedSV_SystemAlert"..i.."MoneyInputFrameSilver"];
		alert.copper = _G["InkedSV_SystemAlert"..i.."MoneyInputFrameCopper"];
		alert.buttons = {}
		for b = 1, 3 do
			local button = _G["InkedSV_SystemAlert"..i.."Button"..b];
			button:SetScript("OnClick", AlertButton_OnClick)
			alert.buttons[b] = button
		end
		_G["InkedSV_SystemAlert"..i.."ItemFrameNameFrame"]:Die()
		_G["InkedSV_SystemAlert"..i.."ItemFrame"]:GetNormalTexture():Die()
		_G["InkedSV_SystemAlert"..i.."ItemFrame"]:SetStyle("Button")
		_G["InkedSV_SystemAlert"..i.."ItemFrameIconTexture"]:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
		_G["InkedSV_SystemAlert"..i.."ItemFrameIconTexture"]:InsetPoints()
	end

	InkedSV_AlertFrame:SetSize(180, 20);
	InkedSV_AlertFrame.callbackOnEnter = true;
	SV:NewAnchor(InkedSV_AlertFrame, L["Loot / Alert Frames"], AlertFramePostMove_Hook)
