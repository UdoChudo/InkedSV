...
local ThreatMeter = _G["InkedSV_ThreatOMeter"];

--[[ LOCALS ]]--
local BARFILE = [[Interface\AddOns\InkedSV\assets\textures\Doodads\THREAT-BAR]];
local TEXTUREFILE = [[Interface\AddOns\InkedSV\assets\textures\Doodads\THREAT-BAR-ELEMENTS]];
...
local RaidMarkFrame = _G["InkedSV_RaidMarkFrame"];
...
local ThreatBar_OnEvent = function(self, event)
	...
	if(scaledPercent and (scaledPercent > 0)) then
		-- if InkedSV is installed then fade instead of show
		if(self.FadeIn) then
			self:FadeIn()
		else
			self:Show()
		...
		-- if InkedSV is installed then fade instead of hide
...
local function InitializeMisc()
	...
	local cfg = CreateFrame("Button", "GameMenuButtonInkedSV", GameMenuFrame, "GameMenuButtonTemplate")
	...
	cfg:SetText("|cffFF9900Inked Shadow Vaults|r")
