...
local Layout = _G["InkedSV_Layout"];
Layout.Frames = {};
Layout.Sections = {
	['ALL'] = {},
	['GENERAL'] = {}
...
local function SnapStickyFrame(frameA, frameB, left, top, right, bottom)
	...
	if (bA  <= tB and bB  <= tA) then
		...
		if lA  <= (lB  +  Sticky.rangeX) and lA  >= (lB - Sticky.rangeX) then
			newX = lB  +  wA
			if frameB == UIParent or frameB == WorldFrame or frameB == InkedSVParent then
				newX = newX  +  4
			end
			snap = true
		end
...
TheHand.bg:SetTexture([[Interface\AddOns\InkedSV\assets\textures\Doodads\MENTALO-HAND-OFF]])
TheHand.energy = TheHand:CreateTexture(nil, "OVERLAY")
TheHand.energy:SetAllPoints(TheHand)
TheHand.energy:SetTexture([[Interface\AddOns\InkedSV\assets\textures\Doodads\MENTALO-ENERGY]])
...
function TheHand:Enable()
	self:Show()
	self.bg:SetTexture([[Interface\AddOns\InkedSV\assets\textures\Doodads\MENTALO-HAND-ON]])
	self.energy:Show()
	self.flash:Play()
...
function TheHand:Disable()
	self.flash:Stop()
	self.energy:Hide()
	self.bg:SetTexture([[Interface\AddOns\InkedSV\assets\textures\Doodads\MENTALO-HAND-OFF]])
...
local function SetPrecisionSizes()
	...
	if(not CurrentFrameTarget.postsize) then
		InkedSV_LayoutPrecision:SetHeight(70)
		InkedSV_LayoutPrecisionWidthAdjust:Hide()
		InkedSV_LayoutPrecisionHeightAdjust:Hide()
	else
		local x,y = InkedSV_LayoutPrecision:GetCenter()
		InkedSV_LayoutPrecision:ClearAllPoints()
		InkedSV_LayoutPrecision:SetPoint("BOTTOMLEFT", InkedSVParent, "BOTTOMLEFT", x,y)

		InkedSV_LayoutPrecision:SetHeight(144)
		...
		InkedSV_LayoutPrecisionWidthAdjust.rangeLow:SetText(minRange);
		InkedSV_LayoutPrecisionWidthAdjust.rangeHigh:SetText(maxRange);
		InkedSV_LayoutPrecisionWidthAdjust:SetMinMaxValues(minRange, maxRange);
		InkedSV_LayoutPrecisionWidthAdjust:SetValue(curWidth);
		InkedSV_LayoutPrecisionWidthAdjust.rangeValue:SetText(curWidth);
		InkedSV_LayoutPrecisionWidthAdjust:Show()
		InkedSV_LayoutPrecisionHeightAdjust.rangeLow:SetText(min2Range);
		InkedSV_LayoutPrecisionHeightAdjust.rangeHigh:SetText(max2Range);
		InkedSV_LayoutPrecisionHeightAdjust:SetMinMaxValues(minRange, maxRange);
		InkedSV_LayoutPrecisionHeightAdjust:SetValue(curHeight);
		InkedSV_LayoutPrecisionHeightAdjust.rangeValue:SetText(curHeight);
		InkedSV_LayoutPrecisionHeightAdjust:Show()
	end
end

function Layout:Movable_OnMouseUp()
	if(not InkedSV_LayoutPrecision) then return end;
	CurrentFrameTarget = self;
	local xOffset, yOffset, anchor = CalculateOffsets()

	InkedSV_LayoutPrecisionSetX.CurrentValue = xOffset;
	InkedSV_LayoutPrecisionSetX:SetText(xOffset)

	InkedSV_LayoutPrecisionSetY.CurrentValue = yOffset;
	InkedSV_LayoutPrecisionSetY:SetText(yOffset)

	InkedSV_LayoutPrecision.Title:SetText(self.textString)
end
...
function Layout:Movable_OnUpdate()
	...
	if CurrentFrameTarget ~= frame then
		InkedSV_LayoutPrecision:Hide()
		frame:GetScript("OnMouseUp")(frame)
	else
		InkedSV_LayoutPrecision:ClearAllPoints()
		InkedSV_LayoutPrecision:SetPoint(anchor1, frame, anchor2, xOffset, yOffset)
		SetPrecisionSizes()
	end
...
function Layout:Movable_OnDragStop()
	...
	if InkedSV_LayoutPrecision then
...
function Layout:Movable_OnEnter()
	...
	UpdateFrameTarget = self;
	Layout.Portrait:SetTexture([[Interface\AddOns\InkedSV\assets\textures\Doodads\MENTALO-ON]])
...
function Layout:Movable_OnLeave()
	...
	Layout.Portrait:SetTexture([[Interface\AddOns\InkedSV\assets\textures\Doodads\MENTALO-OFF]])
	TheHand:Hide()
	if(not InkedSV_LayoutPrecision) then return end;
	if(CurrentFrameTarget ~= self and not InkedSV_LayoutPrecision:IsShown()) then
		self:SetAlpha(0.4)
	end
end
...
local function SetNewAnchor(frame, moveName, title, postDragFunc)
	...
	local currentCategory = InkedSVLib.CURRENT_SCHEMA;
	...
	mtext:SetFontObject(InkedSV_Font_Default)
...
local InkedSV_LayoutPrecisionResetButton_OnClick = function(self)
	if(not CurrentFrameTarget) then return end
...
local XML_LayoutPrecisionInputX_EnterPressed = function(self)
	...
	if(current) then
		if(CurrentFrameTarget) then
			local xOffset, yOffset, anchor = CalculateOffsets()
			yOffset = tonumber(InkedSV_LayoutPrecisionSetY.CurrentValue)
			CurrentFrameTarget:ClearAllPoints()
			CurrentFrameTarget:SetPoint(anchor, InkedSVParent, anchor, current, yOffset)
...
local XML_LayoutPrecisionInputY_EnterPressed = function(self)
	...
	if(current) then
		if(CurrentFrameTarget) then
			local xOffset, yOffset, anchor = CalculateOffsets()
			xOffset = tonumber(InkedSV_LayoutPrecisionSetX.CurrentValue)
			CurrentFrameTarget:ClearAllPoints()
			CurrentFrameTarget:SetPoint(anchor, InkedSVParent, anchor, xOffset, current)
			SaveAnchor(CurrentFrameTarget.name)
		end
		self.CurrentValue = current
	end
...
local XML_LayoutPrecisionWidthAdjust_OnValueChanged = function(self, widthValue)
	...
	if(CurrentFrameTarget and CurrentFrameTarget.postsize) then
		...
		local heightValue = InkedSV_LayoutPrecisionHeightAdjust:GetValue();
...
local XML_LayoutPrecisionHeightAdjust_OnValueChanged = function(self, heightValue)
	...
	if(CurrentFrameTarget and CurrentFrameTarget.postsize) then
		local frame = CurrentFrameTarget.parent;
		local widthValue = InkedSV_LayoutPrecisionWidthAdjust:GetValue();
		CurrentFrameTarget.postsize(frame, widthValue, heightValue);
	end
end
--[[
...
local function InitializeMovables()
	...
	InkedSV_LayoutLockButton:SetSize(110, 25)
	InkedSV_LayoutLockButton.Left:SetAlpha(0)
	InkedSV_LayoutLockButton.Middle:SetAlpha(0)
	InkedSV_LayoutLockButton.Right:SetAlpha(0)
	InkedSV_LayoutLockButton:SetNormalTexture("")
	InkedSV_LayoutLockButton:SetPushedTexture("")
	InkedSV_LayoutLockButton:SetPushedTexture("")
	InkedSV_LayoutLockButton:SetDisabledTexture("")
	InkedSV_LayoutLockButton:RemoveTextures()
	InkedSV_LayoutLockButton:SetFrameLevel(InkedSV_LayoutLockButton:GetFrameLevel() + 1)
	InkedSV_LayoutLockButton.texture = InkedSV_LayoutLockButton:CreateTexture(nil, "BORDER")
	InkedSV_LayoutLockButton.texture:SetSize(110, 50)
	InkedSV_LayoutLockButton.texture:SetPoint("CENTER", InkedSV_LayoutLockButton, "CENTER", 0, -4)
	InkedSV_LayoutLockButton.texture:SetTexture([[Interface\AddOns\InkedSV\assets\textures\Doodads\QUESTION]])
	InkedSV_LayoutLockButton.texture:SetVertexColor(1, 1, 1)
	InkedSV_LayoutLockButton.texture:SetTexCoord(1, 0, 1, 1, 0, 0, 0, 1)
	InkedSV_LayoutLockButton.text = InkedSV_LayoutLockButton:CreateFontString(nil, "OVERLAY")
	InkedSV_LayoutLockButton.text:SetFont(SV.media.font.caps, 18, "OUTLINE")
	InkedSV_LayoutLockButton.text:SetTextColor(1, 0.5, 0)
	InkedSV_LayoutLockButton.text:SetPoint("CENTER")
	InkedSV_LayoutLockButton.text:SetText("Lock")
	InkedSV_LayoutLockButton:SetScript("OnEnter", function(this)
		this.texture:SetVertexColor(0.1, 0.1, 0.1)
		this.text:SetTextColor(1, 1, 0)
	end)
	InkedSV_LayoutLockButton:SetScript("OnLeave", function(this)
		this.texture:SetVertexColor(1, 1, 1)
		this.text:SetTextColor(1, 0.5, 0)
	end)
	InkedSV_LayoutLockButton:SetScript("OnClick", XML_LayoutLockButton_OnClick)

	InkedSV_LayoutGridButton:SetSize(110, 25)
	InkedSV_LayoutGridButton.Left:SetAlpha(0)
	InkedSV_LayoutGridButton.Middle:SetAlpha(0)
	InkedSV_LayoutGridButton.Right:SetAlpha(0)
	InkedSV_LayoutGridButton:SetNormalTexture("")
	InkedSV_LayoutGridButton:SetPushedTexture("")
	InkedSV_LayoutGridButton:SetPushedTexture("")
	InkedSV_LayoutGridButton:SetDisabledTexture("")
	InkedSV_LayoutGridButton:RemoveTextures()
	InkedSV_LayoutGridButton:SetFrameLevel(InkedSV_LayoutGridButton:GetFrameLevel() + 1)
	InkedSV_LayoutGridButton.texture = InkedSV_LayoutGridButton:CreateTexture(nil, "BORDER")
	InkedSV_LayoutGridButton.texture:SetSize(110, 50)
	InkedSV_LayoutGridButton.texture:SetPoint("CENTER", InkedSV_LayoutGridButton, "CENTER", 0, -4)
	InkedSV_LayoutGridButton.texture:SetTexture([[Interface\AddOns\InkedSV\assets\textures\Doodads\QUESTION]])
	InkedSV_LayoutGridButton.texture:SetVertexColor(1, 1, 1)
	InkedSV_LayoutGridButton.text = InkedSV_LayoutGridButton:CreateFontString(nil, "OVERLAY")
	InkedSV_LayoutGridButton.text:SetFont(SV.media.font.caps, 18, "OUTLINE")
	InkedSV_LayoutGridButton.text:SetTextColor(1, 0.5, 0)
	InkedSV_LayoutGridButton.text:SetPoint("CENTER")
	InkedSV_LayoutGridButton.text:SetText("Grid")
	InkedSV_LayoutGridButton:SetScript("OnEnter", function(this)
		this.texture:SetVertexColor(0.1, 0.1, 0.1)
		this.text:SetTextColor(1, 1, 0)
	end)
	InkedSV_LayoutGridButton:SetScript("OnLeave", function(this)
		this.texture:SetVertexColor(1, 1, 1)
		this.text:SetTextColor(1, 0.5, 0)
	end)
	InkedSV_LayoutGridButton:SetScript("OnClick", XML_LayoutGridButton_OnClick)

	InkedSV_LayoutPrecision:SetFrameLevel(999)
	InkedSV_LayoutPrecision:SetStyle("Frame", "Pattern")
	InkedSV_LayoutPrecision:EnableMouse(true)
	InkedSV_LayoutPrecision:RegisterForDrag("LeftButton")

	SV.API:Set("CloseButton", InkedSV_LayoutPrecisionCloseButton)

	InkedSV_LayoutPrecisionSetX:SetStyle("Editbox")
	InkedSV_LayoutPrecisionSetX.CurrentValue = 0;
	InkedSV_LayoutPrecisionSetX:SetScript("OnEnterPressed", XML_LayoutPrecisionInputX_EnterPressed)
	InkedSV_LayoutPrecisionSetY:SetStyle("Editbox")
	InkedSV_LayoutPrecisionSetY.CurrentValue = 0;
	InkedSV_LayoutPrecisionSetY:SetScript("OnEnterPressed", XML_LayoutPrecisionInputY_EnterPressed)
	InkedSV_LayoutPrecisionResetButton:SetStyle("Button")
	InkedSV_LayoutPrecisionUpButton:SetStyle("Button")
	InkedSV_LayoutPrecisionDownButton:SetStyle("Button")
	InkedSV_LayoutPrecisionLeftButton:SetStyle("Button")
	InkedSV_LayoutPrecisionRightButton:SetStyle("Button")
	InkedSV_LayoutPrecisionResetButton:SetScript("OnClick", InkedSV_LayoutPrecisionResetButton_OnClick)

	InkedSV_LayoutPrecisionWidthAdjust:SetValueStep(1);
	InkedSV_LayoutPrecisionHeightAdjust:SetValueStep(1);
	InkedSV_LayoutPrecisionWidthAdjust:SetScript("OnValueChanged", XML_LayoutPrecisionWidthAdjust_OnValueChanged)
	InkedSV_LayoutPrecisionHeightAdjust:SetScript("OnValueChanged", XML_LayoutPrecisionHeightAdjust_OnValueChanged)

	InkedSV_LayoutPrecision:SetScript("OnHide", function()
		if(not CurrentFrameTarget) then return end
