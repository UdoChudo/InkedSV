...
SV.Comix = _G["InkedSV_ComixFrame"];
local COMIX_DATA = {
	{
		{0,0.25,0,0.25},
		{0.25,0.5,0,0.25},
...
--/script InkedSV.Comix:LaunchPremium()
...
--/script InkedSV.Comix:LaunchPopup()
function SV.Comix:LaunchPopup()
	...
	if((rng == 32) and (SV.db.FunStuff.comix == '1')) then
		...
		PlaySoundFile([[Interface\AddOns\InkedSV\assets\sounds\toasty.mp3]])
	elseif(rng < 24) then
...
SV.Drunk = _G["InkedSV_BoozedUpFrame"];
local WORN_ITEMS = {};
...
function SV.Drunk:LetsParty()
	...
	StopMusic()
	PlayMusic([[Interface\AddOns\InkedSV\assets\sounds\beer30.mp3]])
	self:Show()
...
SV.GameMenu = _G["InkedSV_GameMenuFrame"];
--[[
4    - walk
5    - run
26   - attack stance
...
local function InitializeFunStuff()
	...
	narr:SetTexture([[Interface\AddOns\InkedSV\assets\textures\Doodads\AFK-NARRATIVE]])
	...
	splash:SetTexture([[Interface\AddOns\InkedSV\assets\textures\Doodads\PLAYER-AFK]])
	...
	comix.Basic = _G["InkedSV_ComixPopup1"]
	comix.Deluxe = _G["InkedSV_ComixPopup2"]
	comix.Premium = _G["InkedSV_ComixPopup3"]
	...
	toasty.tex:SetTexture([[Interface\AddOns\InkedSV\assets\textures\Doodads\TOASTY]])
	...
	npc.InfoTop:SetBackdrop({
		bgFile = [[Interface\AddOns\InkedSV\assets\textures\NPC-NAMETAG]],
	    tile = false,
	    tileSize = 0,
