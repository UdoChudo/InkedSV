...
MOD.Templates = {
    ["Default"]     = "InkedSV_CoreStyle_Default",
    ["Transparent"] = "InkedSV_CoreStyle_Transparent",
    ["Button"]      = "InkedSV_CoreStyle_Button",
    ["EditBox"]     = "InkedSV_CoreStyle_EditBox",
    ["CheckButton"] = "InkedSV_CoreStyle_CheckButton",
    ["DockButton"]  = "InkedSV_CoreStyle_DockButton",
    ["ActionSlot"]  = "InkedSV_CoreStyle_ActionSlot",
    ["Lite"]        = "InkedSV_CoreStyle_Lite",
    ["Icon"]        = "InkedSV_CoreStyle_Icon",
    ["Bar"]         = "InkedSV_CoreStyle_Bar",
    ["Inset"]       = "InkedSV_CoreStyle_Inset",
    ["Blackout"]    = "InkedSV_CoreStyle_Blackout",
    ["Component"]   = "InkedSV_CoreStyle_Component",
    ["Paper"]       = "InkedSV_CoreStyle_Paper",
    ["Container"]   = "InkedSV_CoreStyle_Container",
    ["Pattern"]     = "InkedSV_CoreStyle_Pattern",
    ["Premium"]     = "InkedSV_CoreStyle_Premium",
    ["Model"]       = "InkedSV_CoreStyle_Model",
    ["UnitLarge"]   = "InkedSV_CoreStyle_UnitLarge",
    ["UnitSmall"]   = "InkedSV_CoreStyle_UnitSmall",
    ["Window"]      = "InkedSV_CoreStyle_Window",
    ["Window2"]     = "InkedSV_CoreStyle_Window2",
};
MOD.Methods = {};
MOD.Concepts = {};
--[[
...
local function ScreenUpdate()
    ...
    if(IsMacClient()) then
        if(not MAC_DISPLAY) then
            MAC_DISPLAY = InkedSVLib:NewGlobal("Display");
...
MOD.Methods["ShadowBox"] = function(self, frame, inverse, styleName, noupdate, overridePadding, xOffset, yOffset, defaultColor)
    ...
    local shadowbox = CreateFrame('Frame', nil, frame, "InkedSV_ShadowBoxTemplate")
...
MOD.Concepts["IconButton"] = function(self, adjustable, frame, texture)
    ...
    if not frame.icon then
        ...
        frame.icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
    end
end
...
MOD.Concepts["ItemButton"] = function(self, adjustable, frame, noScript)
    ...
    if(link) then
        ...
        if(iconObject) then
            iconObject:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
        ...
        if(countObject) then
            ...
            countObject:SetFontObject(InkedSV_Font_Number)
        ...
        if(levelObject) then
            ...
            levelObject:SetFontObject(InkedSV_Font_Number)
...
MOD.Concepts["Tab"] = function(self, adjustable, frame, addBackground, xOffset, yOffset)
    ...
    if(addBackground) then
        ...
        if(nTex) then
            nTex:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
...
MOD.Concepts["QuestItem"] = function(self, adjustable, frame)
    ...
    if(icon) then
        ...
        iconFallback:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))

        if(oldIcon) then
            icon:SetTexture(oldIcon)
        end
        icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
    end
end
...
local ALERT_TEMPLATE = {
    ["typeA"] = {
        COLOR   = {0.8, 0.2, 0.2},
        BG      = [[Interface\AddOns\InkedSV\assets\textures\Alert\ALERT-BG]],
        LEFT    = [[Interface\AddOns\InkedSV\assets\textures\Alert\ALERT-LEFT]],
        RIGHT   = [[Interface\AddOns\InkedSV\assets\textures\Alert\ALERT-RIGHT]],
        TOP     = [[Interface\AddOns\InkedSV\assets\textures\Alert\ALERT-TOP]],
        BOTTOM  = [[Interface\AddOns\InkedSV\assets\textures\Alert\ALERT-BOTTOM]],
    },
    ["typeB"] = {
        COLOR   = {0.08, 0.4, 0},
        BG      = [[Interface\AddOns\InkedSV\assets\textures\Alert\ALERT-BG-2]],
        LEFT    = [[Interface\AddOns\InkedSV\assets\textures\Alert\ALERT-LEFT-2]],
        RIGHT   = [[Interface\AddOns\InkedSV\assets\textures\Alert\ALERT-RIGHT-2]],
        TOP     = [[Interface\AddOns\InkedSV\assets\textures\Alert\ALERT-TOP]],
        BOTTOM  = [[Interface\AddOns\InkedSV\assets\textures\Alert\ALERT-BOTTOM]],
    },
};
...
MOD.Concepts["Alert"] = function(self, defaultStyle, frame, arg)
    ...
    if(not defaultStyle) then
        ...
        alertpanel.bg:SetTexture([[Interface\AddOns\InkedSV\assets\textures\Alert\ALERT-FULL]])
        alertpanel.bg:SetGradient('VERTICAL', 0, 0, 0, .37, .32, .29)

        --[[ ICON BG ]]--
        alertpanel.icon = alertpanel:CreateTexture(nil, "BACKGROUND", nil, -2)
        alertpanel.icon:SetTexture([[Interface\AddOns\InkedSV\assets\textures\Alert\ALERT-ICON-BORDER]])
