...
local function MirrorBarRegistry(barType)
	...
	bar.text:SetFontObject(InkedSV_Font_Default)
...
local function SetTimerStyle(bar)
	for i=1, bar:GetNumRegions()do
		...
		elseif child:GetObjectType() == "FontString" then
			child:SetFontObject(InkedSV_Font_Default)
		end
	end
	bar:SetStatusBarTexture(SV.media.statusbar.gradient)
...
local InkedSV_LootFrameHolder = CreateFrame("Frame", "InkedSV_LootFrameHolder", UIParent);
InkedSV_LootFrameHolder:SetPoint("BOTTOMRIGHT", InkedSV_DockTopLeft, "BOTTOMRIGHT", 0, 0);
InkedSV_LootFrameHolder:SetSize(150, 22);
InkedSV_LootFrameHolder:SetFrameStrata("FULLSCREEN_DIALOG");
InkedSV_LootFrameHolder:SetToplevel(true);

local InkedSV_LootFrame = CreateFrame('Button', 'InkedSV_LootFrame', InkedSV_LootFrameHolder);
InkedSV_LootFrame:SetClampedToScreen(true);
InkedSV_LootFrame:SetPoint('TOPLEFT');
InkedSV_LootFrame:SetSize(256, 64);
InkedSV_LootFrame.title = InkedSV_LootFrame:CreateFontString(nil,'OVERLAY');
InkedSV_LootFrame.title:SetPoint('BOTTOMLEFT',InkedSV_LootFrame,'TOPLEFT',0,1);
InkedSV_LootFrame.slots = {};
InkedSV_LootFrame:Hide();
InkedSV_LootFrame:SetScript("OnHide", function(self)
...
local function HandleSlots(frame)
	...
	for i = 1, #frame.slots do
		...
		if slot:IsShown() then
			counter = counter + 1;
			slot:SetPoint("TOP", InkedSV_LootFrame, 4, (-8 + scale) - (counter * scale))
		end
	end
...
local function MakeSlots(id)
	...
	local slot = CreateFrame("Button", "InkedSV_LootSlot"..id, InkedSV_LootFrame)
	...
	slot.icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
	...
	slot.count:SetFontObject(InkedSV_Font_Loot_Number)
	...
	slot.name:SetFontObject(InkedSV_Font_Loot)
	...
	slot.questTexture:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))

	InkedSV_LootFrame.slots[id] = slot;
	return slot
end
...
local function CreateRollButton(rollFrame, type, locale, anchor)
	...
	text:SetFontObject(InkedSV_Font_Roll)
...
local function CreateRollFrame()
	...
	rollFrame.bindText:SetFontObject(InkedSV_Font_Roll_Number)
	rollFrame.lootText = rollFrame:CreateFontString(nil,"ARTWORK")
	rollFrame.lootText:SetFontObject(InkedSV_Font_Roll_Number)
	...
	rollFrame.yourRoll:SetFontObject(InkedSV_Font_Roll_Number)
...
local function FetchRollFrame()
	...
	local anchorParent = rollFrames[rollCount] or InkedSV_AlertFrame;
...
EventFunc["LOOT_SLOT_CLEARED"] = function(slot)
	if not InkedSV_LootFrame:IsShown() then return; end
	InkedSV_LootFrame.slots[slot]:Hide()
	HandleSlots(InkedSV_LootFrame)
end

EventFunc["LOOT_CLOSED"] = function(...)
	StaticPopup_Hide("LOOT_BIND")
	InkedSV_LootFrame:Hide()
	for _,slot in pairs(InkedSV_LootFrame.slots)do
		slot:Hide()
	end
end

EventFunc["OPEN_MASTER_LOOT_LIST"] = function(...)
	ToggleDropDownMenu(1, nil, GroupLootDropDown, InkedSV_LootFrame.slots[lastID], 0, 0)
end
...
EventFunc["LOOT_READY"] = function(autoLoot)
	...
	if drops > 0 then
		InkedSV_LootFrame:Show()
	else
		CloseLoot(autoLoot == 0)
	end

	if IsFishingLoot() then
		InkedSV_LootFrame.title:SetText(L["Fishy Loot"])
	elseif not UnitIsFriend("player", "target") and UnitIsDead"target" then
		InkedSV_LootFrame.title:SetText(UnitName("target"))
	else
		InkedSV_LootFrame.title:SetText(LOOT)
	end

	if GetCVar("lootUnderMouse") == "1" then
		local cursorX,cursorY = GetCursorPosition()
		cursorX = cursorX / InkedSV_LootFrame:GetEffectiveScale()
		cursorY = (cursorY / (InkedSV_LootFrame:GetEffectiveScale()));
		InkedSV_LootFrame:ClearAllPoints()
		InkedSV_LootFrame:SetPoint("TOPLEFT", nil, "BOTTOMLEFT", cursorX - 40, cursorY + 20)
		InkedSV_LootFrame:GetCenter()
	else
		InkedSV_LootFrame:ClearAllPoints()
		InkedSV_LootFrame:SetPoint("TOPLEFT", InkedSV_LootFrameHolder, "TOPLEFT")
	end

	InkedSV_LootFrame:Raise()

	local iQuality, nameWidth, titleWidth = 0, 0, InkedSV_LootFrame.title:GetStringWidth()
	UpdateLootUpvalues()
	if drops > 0 then
		for i = 1, drops do
			local slot = InkedSV_LootFrame.slots[i] or MakeSlots(i)
		...
		local slot = InkedSV_LootFrame.slots[1] or MakeSlots(1)
	...
	InkedSV_LootFrame:SetBackdropBorderColor(color.r, color.g, color.b, .8)
	InkedSV_LootFrame:SetWidth(max(nameWidth, titleWidth))
end
...
local InkedSV_BailOut = CreateFrame("Button", "InkedSV_BailOut", UIParent)
InkedSV_BailOut:SetSize(50, 50)
InkedSV_BailOut:SetPoint("TOP", InkedSV_DockTopCenter, "BOTTOM", 0, -10)
-- JV - 20160923: This is showing sometimes when it shouldn't so make sure it's hidden by default
InkedSV_BailOut:Hide()
...
local InkedSV_WorldStateHolder = CreateFrame("Frame", "InkedSV_WorldStateHolder", UIParent)
InkedSV_WorldStateHolder:SetPoint("TOP", InkedSV_DockTopCenter, "BOTTOM", 0, -10)
InkedSV_WorldStateHolder:SetSize(200, 45)

local InkedSV_AltPowerBar = CreateFrame("Frame", "InkedSV_AltPowerBar", UIParent)
InkedSV_AltPowerBar:SetPoint("TOP", InkedSV_DockTopCenter, "BOTTOM", 0, -60)
InkedSV_AltPowerBar:SetSize(128, 50)
...
local CaptureBarHandler = function()
	local lastFrame = InkedSV_WorldStateHolder
	local offset = "TOP";
	...
	if(NUM_EXTENDED_UI_FRAMES) then
		for i=1, NUM_EXTENDED_UI_FRAMES do
			...
			local frame = _G[name]
			if(frame and frame:IsVisible()) then
				if(_G[name .. "LeftBar"]) then _G[name .. "LeftBar"]:SetTexture("Interface\\AddOns\\InkedSV\\assets\\textures\\WorldState-CaptureBar") end
				if(_G[name .. "RightBar"]) then _G[name .. "RightBar"]:SetTexture("Interface\\AddOns\\InkedSV\\assets\\textures\\WorldState-CaptureBar") end
				if(_G[name .. "MiddleBar"]) then _G[name .. "MiddleBar"]:SetTexture("Interface\\AddOns\\InkedSV\\assets\\textures\\WorldState-CaptureBar") end
				if(_G[name .. "LeftLine"]) then _G[name .. "LeftLine"]:SetTexture("Interface\\AddOns\\InkedSV\\assets\\textures\\WorldState-CaptureBar") end
				if(_G[name .. "RightLine"]) then _G[name .. "RightLine"]:SetTexture("Interface\\AddOns\\InkedSV\\assets\\textures\\WorldState-CaptureBar") end
				if(_G[name .. "LeftIconHighlight"]) then _G[name .. "LeftIconHighlight"]:SetTexture("Interface\\AddOns\\InkedSV\\assets\\textures\\WorldState-CaptureBar") end
				if(_G[name .. "RightIconHighlight"]) then _G[name .. "RightIconHighlight"]:SetTexture("Interface\\AddOns\\InkedSV\\assets\\textures\\WorldState-CaptureBar") end
				if(_G[name .. "IndicatorLeft"]) then _G[name .. "IndicatorLeft"]:SetTexture("Interface\\AddOns\\InkedSV\\assets\\textures\\WorldState-CaptureBar") end
				if(_G[name .. "IndicatorRight"]) then _G[name .. "IndicatorRight"]:SetTexture("Interface\\AddOns\\InkedSVe\\assets\\textures\\WorldState-CaptureBar") end
				frame:ClearAllPoints()
				frame:SetPoint("TOP", lastFrame, offset, 0, 0)
				lastFrame = frame
				offset = "BOTTOM";
...
local PlayerPowerBarAlt_OnClearAllPoints = function(self)
		self:SetPoint("CENTER", InkedSV_AltPowerBar, "CENTER", 0, 0)
end
...
local function SetOverrides()
	...
	InkedSV_WorldStateHolder:SetSize(200, 45)
	SV:NewAnchor(InkedSV_WorldStateHolder, L["Capture Bars"])
	NewHook("UIParent_ManageFramePositions", CaptureBarHandler)
	--WorldStateAlwaysUpFrame:ClearAllPoints()
	--WorldStateAlwaysUpFrame:SetPoint("TOP",InkedSV_WorldStateHolder,"TOP",0,0)

	InkedSV_AltPowerBar:SetSize(128, 50)
	PlayerPowerBarAlt:ClearAllPoints()
	PlayerPowerBarAlt:SetPoint("CENTER", InkedSV_AltPowerBar, "CENTER", 0, 0)
	PlayerPowerBarAlt:SetParent(InkedSV_AltPowerBar)
	PlayerPowerBarAlt.ignoreFramePositionManager = true;
	NewHook(PlayerPowerBarAlt, "ClearAllPoints", PlayerPowerBarAlt_OnClearAllPoints)
	SV:NewAnchor(InkedSV_AltPowerBar, L["Alternative Power"])

	if(InkedSV_Player) then
		InkedSV_BailOut:ClearAllPoints()
		local size = InkedSV_Player:GetHeight()
		InkedSV_BailOut:SetSize(size, size)
		InkedSV_BailOut:SetPoint("TOPLEFT", InkedSV_Player, "TOPRIGHT", 4, 0)
		InkedSV_BailOut:Hide()
	end
	InkedSV_BailOut:SetNormalTexture(SV.media.icon.exitIcon)
	InkedSV_BailOut:SetPushedTexture(SV.media.icon.exitIcon)
	InkedSV_BailOut:SetHighlightTexture(SV.media.icon.exitIcon)
	InkedSV_BailOut:SetStyle("!_Frame", "Transparent")
	InkedSV_BailOut:RegisterForClicks("AnyUp")
	InkedSV_BailOut:SetScript("OnClick", BailOut_OnClick)
	InkedSV_BailOut:RegisterEvent("UNIT_ENTERED_VEHICLE")
 	InkedSV_BailOut:RegisterEvent("UNIT_EXITED_VEHICLE")
 	InkedSV_BailOut:RegisterEvent("VEHICLE_UPDATE")
 	--InkedSV_BailOut:RegisterEvent("PLAYER_ENTERING_WORLD")
 	InkedSV_BailOut:SetScript("OnEvent", BailOut_OnEvent)
 	NewHook("TakeTaxiNode", BailOut_OnHook)
	SV:NewAnchor(InkedSV_BailOut, L["Bail Out"])
	InkedSV_BailOut:Hide()
	...
	if(SV.db.general.loot) then
		...
		InkedSV_LootFrameHolder:SetSize(150, 22);
		-- SV:NewAnchor(InkedSV_LootFrameHolder, L["Loot Frame"], nil, nil, "InkedSV_LootFrame");
		SV:NewAnchor(InkedSV_LootFrameHolder, L["Loot Frame"]);

		InkedSV_LootFrame:SetSize(256, 64);
		InkedSV_LootFrame:SetStyle("!_Frame", 'Transparent');
		InkedSV_LootFrame.title:SetFontObject(InkedSV_Font_Header)
		SV:ManageVisibility(InkedSV_LootFrame);
		InkedSV_LootFrame:Hide();
