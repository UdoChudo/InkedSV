...
local SchemaFromMeta        = "X-Schema";
local HeaderFromMeta        = "X-Name";
local ThemeFromMeta         = "X-Theme";
...
local GLOBAL_Inked, PRIVATE_Inked, FILTER_Inked, MEDIA_Inked, ERROR_Inked, SHARED_Inked;
...
local function CopySharedData(key)
  if((not key) or (not SHARED_Inked.profiles[key])) then return end
  local export = SHARED_Inked.profiles[key];
...
local function SaveSharedData()
  for schema,data in pairs(PRIVATE_Inked.SAFEDATA.SHARED) do
    local obj = CoreObject[schema];
    if(data.enabled and obj and obj.private) then
      SHARED_Inked.profiles[PROFILE_KEY][schema] = {};
      for k,v in pairs(obj.private) do
        if(not data.ignored[k]) then
          if(not SHARED_Inked.profiles[PROFILE_KEY][schema][k]) then
            SHARED_Inked.profiles[PROFILE_KEY][schema][k] = {};
          end
          local saved = SHARED_Inked.profiles[PROFILE_KEY][schema][k];
          sharedcopy(saved, v);
        end
      end
    end
...
local function LoadSharedData(schema,datastore)
  ...
  local data = PRIVATE_Inked.SAFEDATA.SHARED[schema];
  if(data and data.enabled) then
    local export = SHARED_Inked.profiles[PROFILE_KEY][schema];
...
local isSharingEnabled = function(self)
  ...
  if(self.private and PRIVATE_Inked.SAFEDATA.SHARED[schema]) then
    return PRIVATE_Inked.SAFEDATA.SHARED[schema].enabled;
  end
  return false;
end
...
local toggleSharedData = function(self, value)
  ...
  if(self.private) then
    PRIVATE_Inked.SAFEDATA.SHARED[schema].enabled = value;
    if(value) then
      SaveSharedData();
    end
  end
...
local ignoreSharedKeys = function(self, ...)
  ...
  if(self.private) then
    for i=1,select("#",...) do
      local key = select(i,...);
      PRIVATE_Inked.SAFEDATA.SHARED[schema].ignored[key] = true;
    end
  end
end
...
function lib:LiveUpdate(forced)
    if(forced or PRIVATE_Inked.SAFEDATA.NEEDSLIVEUPDATE) then
        ...
        if((not InCombatLockdown()) and (not C_PetBattles.IsInBattle())) then
            PRIVATE_Inked.SAFEDATA.NEEDSLIVEUPDATE = false
        end
    end
end
...
function lib:CleanUpData(deep)
    ...
    if(DEEP_CLEAN_REQUESTED) then
        for key,data in pairs(GLOBAL_Inked.profiles) do
            sanitize(data, defaults, true)
        end
        for theme,realms in pairs(MEDIA_Inked.profiles) do
            if(CoreObject.AvailableThemes[theme]) then
                for key,data in pairs(MEDIA_Inked.profiles[theme]) do
                    if(not GLOBAL_Inked.profileKeys[key]) then
                        MEDIA_Inked.profiles[theme][key] = nil
                    else
                        sanitize(data, media)
                    end
                end
    ...
    elseif(not deep) then
        for key,data in pairs(GLOBAL_Inked.profiles) do
            ...
            if(linked) then
                sanitize(data, defaults, true)
                sanitize(MEDIA_Inked.profiles[linked][key], media)
            end
        end
    else
...
local function CheckForDeprecated(oldKey)
  if(GLOBAL_Inked.profiles[oldKey]) then
    local export = GLOBAL_Inked.profiles[oldKey];
    local saved = GLOBAL_Inked.profiles[PROFILE_KEY];
    tablecopy(saved, export);
    GLOBAL_Inked.profiles[oldKey] = nil
  end

  if(MEDIA_Inked.profiles[THEME_KEY][oldKey]) then
    local export = MEDIA_Inked.profiles[THEME_KEY][oldKey];
    local saved = MEDIA_Inked.profiles[THEME_KEY][PROFILE_KEY];
    tablecopy(saved, export);
    MEDIA_Inked.profiles[THEME_KEY][oldKey] = nil
  end
end
...
local function GenerateProfileKey(key)
    local safeKey = key;
    if(not safeKey) then
        safeKey = PRIVATE_Inked.SAFEDATA.CurrentProfile or ("%s - %s"):format(playerName, "Default");
    end
    key = key or "Default";
    if(not GLOBAL_Inked.profileRealms[safeKey]) then
        GLOBAL_Inked.profileRealms[safeKey] = playerRealm;
    elseif(safeKey:find(playerName) and GLOBAL_Inked.profileRealms[safeKey] ~= playerRealm) then
    ...
    PRIVATE_Inked.SAFEDATA.CurrentProfile = safeKey
...
local function UpdateProfileSources(newKey)
    ...
    if(PRIVATE_Inked.SAFEDATA.DUALSPEC) then
    ...
    if(not GLOBAL_Inked.profiles[PROFILE_KEY]) then GLOBAL_Inked.profiles[PROFILE_KEY] = {} end
    if(not GLOBAL_Inked.profileKeys[PROFILE_KEY]) then
        for k,v in pairs(GLOBAL_Inked.profiles) do
            GLOBAL_Inked.profileKeys[k] = k
        end
    end
    if(not SHARED_Inked.profiles[PROFILE_KEY]) then
      SHARED_Inked.profiles[PROFILE_KEY] = {};
    end
    if(PRIVATE_Inked.SAFEDATA.THEME) then
        THEME_KEY = PRIVATE_Inked.SAFEDATA.THEME;
    else
        THEME_KEY = DEFAULT_THEME_KEY;
    end
    if(not MEDIA_Inked.profiles[THEME_KEY]) then MEDIA_Inked.profiles[THEME_KEY] = {} end
    if(not MEDIA_Inked.profiles[THEME_KEY][PROFILE_KEY]) then MEDIA_Inked.profiles[THEME_KEY][PROFILE_KEY] = {} end
...
local function UpdateConstraints()
    for k,v in pairs(PRIVATE_Inked.SAFEDATA.SAVED) do
...
local function UpdateCoreDatabases()
    ...
    private.data       = PRIVATE_Inked
    CoreObject.private = private

    CoreObject.ERRORLOG = ERROR_Inked.FOUND
...
local function CorePreInitialize()
    ...
    PRIVATE_Inked = _G[PRIVATE_FILENAME]
    --PROFILE SAFE VARIABLES
    if(not PRIVATE_Inked.SAFEDATA) then PRIVATE_Inked.SAFEDATA = {} end
    if(not PRIVATE_Inked.SAFEDATA.SAVED) then PRIVATE_Inked.SAFEDATA.SAVED = {} end
    if(not PRIVATE_Inked.SAFEDATA.SHARED) then PRIVATE_Inked.SAFEDATA.SHARED = {} end
    if(not PRIVATE_Inked.SAFEDATA.THEME) then PRIVATE_Inked.SAFEDATA.THEME = DEFAULT_THEME_KEY end
    if(not PRIVATE_Inked.SAFEDATA.DUALSPEC) then PRIVATE_Inked.SAFEDATA.DUALSPEC = false end
    if(not PRIVATE_Inked.SAFEDATA.NEEDSLIVEUPDATE) then PRIVATE_Inked.SAFEDATA.NEEDSLIVEUPDATE = false end
    --GLOBAL SAVED VARIABLES
    if(not _G[GLOBAL_FILENAME]) then _G[GLOBAL_FILENAME] = {} end
    GLOBAL_Inked = _G[GLOBAL_FILENAME]
    --GLOBAL PROFILE DATA
    if(not GLOBAL_Inked.profiles) then GLOBAL_Inked.profiles = {} end
    if(not GLOBAL_Inked.profiles[PROFILE_KEY]) then GLOBAL_Inked.profiles[PROFILE_KEY] = {} end
    if(not GLOBAL_Inked.profileRealms) then GLOBAL_Inked.profileRealms = {} end
    if(not GLOBAL_Inked.profileRealms[PROFILE_KEY]) then GLOBAL_Inked.profileRealms[PROFILE_KEY] = playerRealm end
    if(not GLOBAL_Inked.SAFEDATA) then GLOBAL_Inked.SAFEDATA = {} end
    if(not GLOBAL_Inked.SAFEDATA.MasterProfile) then GLOBAL_Inked.SAFEDATA.MasterProfile = false; end
    --GLOBAL KEY STORAGE (ALWAYS EMPTY ON LOGIN)
    GLOBAL_Inked.profileKeys = {}
    --SAVED ERRORS
    if not _G[ERROR_FILENAME] then _G[ERROR_FILENAME] = {} end
    ERROR_Inked = _G[ERROR_FILENAME]
    --ONLY ALLOW TODAYS ERRORS
    if(ERROR_Inked.TODAY and ERROR_Inked.TODAY ~= DATESTAMP) then
        ERROR_Inked.FOUND = {}
    elseif(not ERROR_Inked.FOUND) then
        ERROR_Inked.FOUND = {}
    end
    --UPDATE THE ERROR DATESTAMP
    ERROR_Inked.TODAY = DATESTAMP
    --MEDIA SAVED VARIABLES
    if not _G[MEDIA_FILENAME] then _G[MEDIA_FILENAME] = {} end
    MEDIA_Inked = _G[MEDIA_FILENAME]
    --MEDIA PROFILE DATA
    if(not MEDIA_Inked.profiles) then MEDIA_Inked.profiles = {} end
    if(not MEDIA_Inked.profiles[THEME_KEY]) then MEDIA_Inked.profiles[THEME_KEY] = {} end
    if(not MEDIA_Inked.profiles[THEME_KEY][PROFILE_KEY]) then MEDIA_Inked.profiles[THEME_KEY][PROFILE_KEY] = {} end
    --SHARED PRIVATE DATA
    if not _G[SHARED_FILENAME] then _G[SHARED_FILENAME] = {} end
    SHARED_Inked = _G[SHARED_FILENAME]
    if(not SHARED_Inked.profiles) then SHARED_Inked.profiles = {} end
    if(not SHARED_Inked.profiles[PROFILE_KEY]) then SHARED_Inked.profiles[PROFILE_KEY] = {} end
    ...
    for i = 1, GetNumAddOns() do
        ...
        if(IsAddOnLoadOnDemand(i)) then
            ...
            if(header and schema) then
                ...
                if(not PRIVATE_Inked.SAFEDATA.SAVED[schema]) then
                    PRIVATE_Inked.SAFEDATA.SAVED[schema] = true
                end
                        ...
                        enable = {
                            ...
                            func = function()
                                if(not IsAddOnLoaded(addonName)) then
                                    ...
                                    PRIVATE_Inked.SAFEDATA.SAVED[schema] = false
                ...
                local enabled = PRIVATE_Inked.SAFEDATA.SAVED[schema]
...
local Library_OnEvent = function(self, event, arg, ...)
    ...
    elseif(event == "PLAYER_LOGIN") then
        ...
        PRIVATE_Inked.SAFEDATA.NEEDSLIVEUPDATE = C_PetBattles.IsInBattle()
    ...
    if MODULES then
        for i=1,#MODULES do
            ...
            if(obj and (not obj.initialized)) then
                ...
                if(not halt) then
                    if(not PRIVATE_Inked.SAFEDATA.SHARED[schema]) then
                        PRIVATE_Inked.SAFEDATA.SHARED[schema] = {
                          enabled = false,
                          ignored = {}
                        };
                    end
                    PRIVATE_Inked.SAFEDATA.SHARED[schema].ignored.SAFEDATA = true;
                    PRIVATE_Inked.SAFEDATA.SHARED[schema].ignored.data = true;
...
function lib:Remove(key)
    if(GLOBAL_Inked.profiles[key]) then GLOBAL_Inked.profiles[key] = nil end
    if(GLOBAL_Inked.profileRealms[key]) then GLOBAL_Inked.profileRealms[key] = nil end
    wipe(GLOBAL_Inked.profileKeys)
    for k,v in pairs(GLOBAL_Inked.profiles) do
        GLOBAL_Inked.profileKeys[k] = k
    end
end
...
function lib:GetProfiles()
    local list = GLOBAL_Inked.profileKeys or {}
    return list
end
...
function lib:CurrentProfile()
    return PRIVATE_Inked.SAFEDATA.CurrentProfile
end

function lib:UnsetProfile()
    PRIVATE_Inked.SAFEDATA.CurrentProfile = nil;
end

function lib:CopyDatabase(key, linked)
  if((not key) or (not GLOBAL_Inked.profiles[key])) then return end
  PRIVATE_Inked.SAFEDATA["install_version"] = CoreObject.Version
  if(not linked) then
    wipe(GLOBAL_Inked.profiles[PROFILE_KEY])
    local export = GLOBAL_Inked.profiles[key];
    local saved = GLOBAL_Inked.profiles[PROFILE_KEY];
    tablecopy(saved, export);

    if(MEDIA_Inked.profiles[THEME_KEY][key]) then
      wipe(MEDIA_Inked.profiles[THEME_KEY][PROFILE_KEY])
      export = MEDIA_Inked.profiles[THEME_KEY][key];
      saved = MEDIA_Inked.profiles[THEME_KEY][PROFILE_KEY];
      tablecopy(saved, export);
    end
...
function lib:CloneDatabase(key)
    ...
    PRIVATE_Inked.SAFEDATA["install_version"] = CoreObject.Version
    local export, saved
    if(not GLOBAL_Inked.profiles[key]) then GLOBAL_Inked.profiles[key] = {} end;
    export = GLOBAL_Inked.profiles[PROFILE_KEY];
    saved = GLOBAL_Inked.profiles[key];
    tablecopy(saved, export);

    if(not MEDIA_Inked.profiles[THEME_KEY][key]) then MEDIA_Inked.profiles[THEME_KEY][key] = {} end;
    export = MEDIA_Inked.profiles[THEME_KEY][PROFILE_KEY];
    saved = MEDIA_Inked.profiles[THEME_KEY][key];
    tablecopy(saved, export);
...
function lib:ExportDatabase()
    ...
    tablecopy(t["PROFILE"], GLOBAL_Inked.profiles[PROFILE_KEY]);
    tablecopy(t["MEDIA"][THEME_KEY], MEDIA_Inked.profiles[THEME_KEY][PROFILE_KEY]);
...
function lib:ImportDatabase(encoded)
    ...
    saved = GLOBAL_Inked.profiles[PROFILE_KEY];
...
function lib:WipeDatabase()
    for k,v in pairs(GLOBAL_Inked.profiles[PROFILE_KEY]) do
        GLOBAL_Inked.profiles[PROFILE_KEY][k] = nil
    end
    for k,v in pairs(MEDIA_Inked.profiles[THEME_KEY][PROFILE_KEY]) do
        MEDIA_Inked.profiles[THEME_KEY][PROFILE_KEY][k] = nil
    end
    for k,v in pairs(SHARED_Inked.profiles[PROFILE_KEY]) do
        SHARED_Inked.profiles[PROFILE_KEY][k] = nil
    end
end

function lib:WipeAllSharedData()
    for k,v in pairs(SHARED_Inked.profiles) do
        SHARED_Inked.profiles[k] = nil
    end
end

function lib:GetSafeData(index)
    if(index) then
        return PRIVATE_Inked.SAFEDATA[index]
    else
        return PRIVATE_Inked.SAFEDATA
    end
end

function lib:SaveSafeData(index, value)
    PRIVATE_Inked.SAFEDATA[index] = value
end

function lib:CheckData(schema, key)
    local file = GLOBAL_Inked.profiles[PROFILE_KEY][schema]
...
function lib:NewGlobal(index)
    ...
    if(not GLOBAL_Inked[index]) then
        GLOBAL_Inked[index] = {}
    end
    return GLOBAL_Inked[index]
end

function lib:CheckDualProfile()
    return PRIVATE_Inked.SAFEDATA.DUALSPEC
end

function lib:ToggleDualProfile(enabled)
    PRIVATE_Inked.SAFEDATA.DUALSPEC = enabled
    UpdateProfileSources()
end

function lib:CheckMasterProfile()
    local mp = GLOBAL_Inked.SAFEDATA.MasterProfile
    if(mp and GLOBAL_Inked.profiles[mp]) then
      return mp
    else
      return false
    end
...
function lib:SetMasterProfile(key)
    if((not key) or (not GLOBAL_Inked.profiles[key])) then
      GLOBAL_Inked.SAFEDATA.MasterProfile = false;
    else
      GLOBAL_Inked.SAFEDATA.MasterProfile = key;
    end
end
