...
SV.Setup = {};
SV.media.button = {
	["option"] 		= [[Interface\AddOns\InkedSV\assets\textures\SETUP-OPTION]],
	["arrow"] 		= [[Interface\AddOns\InkedSV\assets\textures\SETUP-ARROW]],
	["theme"] 		= [[Interface\AddOns\InkedSV\assets\textures\THEME-DEFAULT]]
};
...
local function PlayThemeSong()
	if(not musicIsPlaying) then
		...
		StopMusic()
		PlayMusic([[Interface\AddOns\InkedSV\assets\sounds\SuperVillain.mp3]])
		musicIsPlaying = true
	end
end
...
local function ShowLayout(show)
	...
	if(not _G["InkedSV_Raid"] or (show and _G["InkedSV_Raid"].forceShow == true)) then return end
	if(not show and _G["InkedSV_Raid"].forceShow ~= true) then return end
	SV.UnitFrames:ViewGroupFrames(_G["InkedSV_Raid"], show)
end

local function ShowAuras(show)
	if(not SV.UnitFrames) then return end
	if(not _G["InkedSV_Player"] or (show and _G["InkedSV_Player"].forceShowAuras)) then return end
	if(not show and not _G["InkedSV_Player"].forceShowAuras) then return end
	_G["InkedSV_Player"].forceShowAuras = show
...
local function BarShuffle()
	...
	SV:ReAnchor("InkedSV_ActionBar2", "BOTTOM", InkedSV_ActionBar1, "TOP", 0, -bS);
	SV:ReAnchor("InkedSV_ActionBar3", "BOTTOMLEFT", InkedSV_ActionBar1, "BOTTOMRIGHT", 4, 0);
	SV:ReAnchor("InkedSV_ActionBar5", "BOTTOMRIGHT", InkedSV_ActionBar1, "BOTTOMLEFT", -4, 0);
	SV:ReAnchor("InkedSV_PetActionBar", "BOTTOMLEFT", InkedSV_ActionBarMainAnchor, "TOPLEFT", 0, 4);
	SV:ReAnchor("InkedSV_StanceBar", "BOTTOMRIGHT", InkedSV_ActionBarMainAnchor, "TOPRIGHT", 0, 4);
	SV:ReAnchor("InkedSV_SpecialAbility", "BOTTOM", InkedSV_ActionBarMainAnchor, "TOP", 0, 75);
end
...
local function UFMoveBottomQuadrant(toggle)
	...
	SV:ReAnchor("InkedSV_Player", "BOTTOMRIGHT", SV.Screen, "BOTTOM", -x, y);
	SV:ReAnchor("InkedSV_PlayerCastbar", "BOTTOMRIGHT", SV.Screen, "BOTTOM", -x, y-60);
	SV:ReAnchor("InkedSV_Pet", "RIGHT", InkedSV_Player, "LEFT", -2, 0);
	SV:ReAnchor("InkedSV_Target", "BOTTOMLEFT", SV.Screen, "BOTTOM", x, y);
	SV:ReAnchor("InkedSV_TargetCastbar", "BOTTOMLEFT", SV.Screen, "BOTTOM", x, y-60);
	SV:ReAnchor("InkedSV_TargetTarget", "LEFT", InkedSV_Target, "RIGHT", 2, 0);
	SV:ReAnchor("InkedSV_Focus", "BOTTOMLEFT", SV.Screen, "BOTTOM", x2, y2);
	SV:ReAnchor("InkedSV_ThreatBar", "BOTTOMRIGHT", SV.Screen, "BOTTOMRIGHT", -x3, y3);
end
...
local function UFMoveLeftQuadrant(toggle)
	...
	SV:ReAnchor("InkedSV_Party", "BOTTOMLEFT", SV.Screen, "BOTTOMLEFT", x, dH);
	SV:ReAnchor("InkedSV_Raid", "BOTTOMLEFT", SV.Screen, "BOTTOMLEFT", x, dH);
	SV:ReAnchor("InkedSV_Raidpet", "TOPLEFT", SV.Screen, "TOPLEFT", x, -y1);
	SV:ReAnchor("InkedSV_Assist", "TOPLEFT", SV.Screen, "TOPLEFT", x, -y2);
	SV:ReAnchor("InkedSV_Tank", "TOPLEFT", SV.Screen, "TOPLEFT", x, -y3);
end
...
local function UFMoveTopQuadrant(toggle)
	...
	SV:ReAnchor("InkedSV_LootFrame", "BOTTOM", SV.Screen, "BOTTOM", 0, y2);
	SV:ReAnchor("InkedSV_AltPowerBar", "TOP", SV.Screen, "TOP", 0, -y3);
...
local function UFMoveRightQuadrant(toggle)
	...
	SV:ReAnchor("InkedSV_BossHolder", "RIGHT", SV.Screen, "RIGHT", -x, 0);
	SV:ReAnchor("InkedSV_ArenaHolder", "RIGHT", SV.Screen, "RIGHT", -x, 0);
	if(SV.Tooltip) then
		SV:ReAnchor("InkedSV_ToolTip", "BOTTOMRIGHT", SV.Screen, "BOTTOMRIGHT", -x2, dH);
	end
end
--[[
##########################################################
...
function SV.Setup:UserScreen(rez, preserve)
	...
	if(not preserve and not mungs) then
		-- BarShuffle()
    SV:UpdateAnchors()
		InkedSVLib:RefreshModule('Dock')
		InkedSVLib:RefreshModule('Auras')
		InkedSVLib:RefreshModule('ActionBars')
		InkedSVLib:RefreshModule('UnitFrames')
...
function SV.Setup:ChatConfigs(mungs)
	...
	if(SV.Chat) then
		...
		for i = 1, NUM_CHAT_WINDOWS do
			...
			FCF_SetChatWindowFontSize(nil, chat, 16)
			if i == 1 then
...
function SV.Setup:RandomBackdrops()
	...
	if(SV.UnitFrames) then InkedSVLib:RefreshModule('UnitFrames') end
end
...
function SV.Setup:ColorTheme(style, preserve)
	...
	InkedSVLib:SaveSafeData("preset_mediastyle", preset_mediastyle);
...
function SV.Setup:UnitframeLayout(style, preserve)
	...
	InkedSVLib:SaveSafeData("preset_unitstyle", preset_unitstyle);
...
function SV.Setup:GroupframeLayout(style, preserve)
	...
	InkedSVLib:SaveSafeData("preset_groupstyle", preset_groupstyle);
...
function SV.Setup:BarLayout(style, preserve)
	...
	InkedSVLib:SaveSafeData("preset_barstyle", preset_barstyle);
...
function SV.Setup:Auralayout(style, preserve)
	...
	InkedSVLib:SaveSafeData("preset_aurastyle", preset_aurastyle);
...
function SV.Setup:EZDefault()
	...
	InkedSVLib:SaveSafeData("install_version", SV.Version)
	StopMusic()
	SetCVar("Sound_MusicVolume", user_music_vol or 0)
	InkedSVLib:SaveSafeData("version", SV.Version)
	ReloadUI()
end
...
function SV.Setup:NewSettings()
	CURRENT_PAGE = 2;
	InkedSV_InstallerFrame:SetPage(CURRENT_PAGE)
end
...
local OptionButton_OnClick = function(self)
	...
	if(self.ClickIndex) then
		for option, text in pairs(self.ClickIndex) do
			InkedSV_InstallerFrame[option]:SetText(text)
		end
	end
...
function SV.Setup:Reset()
	...
	local old = InkedSVLib:GetSafeData()
...
function SV.Setup:Install(autoLoaded)
	local mp = InkedSVLib:CheckMasterProfile();
	if(mp) then
    	SV.SystemAlert["MASTER_PROFILE_PROMPT"].OnAccept = function() InkedSVLib:CopyDatabase(mp) end
...
function SV.Setup:LoadInstaller(autoLoaded)
	...
	if not InkedSV_InstallerFrame then
		local frame = CreateFrame("Button", "InkedSV_InstallerFrame", UIParent)
		...
		titleHolder:SetText(L["InkedSV Installation"])
	...
	InkedSV_InstallerFrame:SetScript("OnHide", function()
		StopMusic()
	...
	InkedSV_InstallerFrame:Show()
	InkedSV_InstallerFrame:SetPage(1)
