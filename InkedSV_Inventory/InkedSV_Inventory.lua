...
local InkedSV_Font_Default 	= _G.InkedSV_Font_Default;
local InkedSV_Font_Header 		= _G.InkedSV_Font_Header;
local InkedSV_Font_Bag 		= _G.InkedSV_Font_Bag;
local InkedSV_Font_Bag_Number 	= _G.InkedSV_Font_Bag_Number;

local BagFilters = _G.InkedSV_BagFilterMenu;
local BagBar = _G.InkedSV_BagBar;
--[[
##########################################################
GET ADDON DATA
##########################################################
	...
	for i, bagID in ipairs(self.BagIDs) do
		if((not isBank and bagID <= 3) or (isBank and bagID ~= -1 and numContainerSlots >= 1 and not (i - 1 > numContainerSlots))) then
			...
			if(not bagSlot) then
				...
				if isBank then
					globalName = "InkedSV_BankBag" .. bagID - 4;
					bagTemplate = "BankItemButtonBagTemplate";
				else
					globalName = "InkedSV_MainBag" .. bagID .. "Slot";
					bagTemplate = "SecureActionButtonTemplate";
				end
				bagSlot = CreateFrame("ItemButton", globalName, menu, bagTemplate)
				bagSlot.parent = self;

				bagSlot:SetNormalTexture("Interface\AddOns\InkedSV\assets\borders\DEFAULT")
				...
				bagSlot.icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
		...
		if numSlots > 0 then
			...
			for slotID = 1, numSlots do
				...
				if not self.Bags[bagID][slotID] then
					...
					self.Bags[bagID][slotID].icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS));
					...
					self.Bags[bagID][slotID].questIcon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS));
				...
				if(not self.Bags[bagID][slotID].GearInfo) then
					...
					self.Bags[bagID][slotID].GearInfo:SetFontObject(InkedSV_Font_Default)
				...
				if(not self.Bags[bagID][slotID].ItemLevel) then
					...
					self.Bags[bagID][slotID].ItemLevel:SetFontObject(InkedSV_Font_Default)
                ...
                if(not self.Bags[bagID][slotID].Count) then
                    ...
                    self.Bags[bagID][slotID].Count:SetFontObject(InkedSV_Font_Default)
...
local ReagentFrame_UpdateLayout = function(self)
	...
	for slotID = 1, maxCount do
		...
		if not bag[slotID] then
			...
			slot.questIcon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
		...
		if(slot.GetInventorySlotInfo) then
...
do
	...
	local function LoadBagBar()
		...
		MainMenuBarBackpackButtonCount:SetFontObject(InkedSV_Font_Default)
...
do
	...
	function MOD:CreateMasterFrame()
		local bagName = "InkedSV_ContainerFrame";
		local frame = CreateFrame("Button", "InkedSV_ContainerFrame", SV.Screen);
		...
		frame.closeButton = CreateFrame("Button", "InkedSV_ContainerFrameCloseButton", frame, "UIPanelCloseButton")
		...
		frame.goldText:SetFontObject(InkedSV_Font_Bag_Number)
		...
		frame.editBox = CreateFrame("EditBox", "InkedSV_ContainerFrameEditBox", frame)
		...
		frame.editBox:SetFontObject(InkedSV_Font_Bag)
		...
		searchText:SetFontObject(InkedSV_Font_Bag)
...
--[[
		...
		for h = 1, MAX_WATCHED_TOKENS do
			...
			frame.currencyButton[h].icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
			...
			frame.currencyButton[h].text:SetFontObject(InkedSV_Font_Bag_Number)
		...
		if(SV.db.Inventory.separateBags) then
			...
			for i, bagID in ipairs(frame.BagIDs) do
				if(bagID > 0) then
					local singleBagFrameName = "InkedSV_ContainerFrameBag" .. bagID;
					...
					elseif(bagID == 2) then
						singleBagFrame:SetPoint("TOPRIGHT", _G['InkedSV_ContainerFrameBag1'], "TOPLEFT", -4, 0)
					elseif(bagID == 3) then
						singleBagFrame:SetPoint("TOPRIGHT", _G['InkedSV_ContainerFrameBag1'], "BOTTOMRIGHT", 0, -4)
					else
						singleBagFrame:SetPoint("TOPRIGHT", _G['InkedSV_ContainerFrameBag3'], "TOPLEFT", -4, 0)
					end
	...
	function MOD:CreateBankOrReagentFrame(isReagent)
		...
		local bagName = isReagent and "InkedSV_ReagentContainerFrame" or "InkedSV_BankContainerFrame"
		...
		if(not isReagent) then
			...
			frame.swapButton:SetScript("OnClick", function()
				if(not IsReagentBankUnlocked()) then
					...
					PlaySound(SOUNDKIT.IG_MAINMENU_OPTION);
					if(_G["InkedSV_ReagentContainerFrame"]:IsShown()) then
						_G["InkedSV_ReagentContainerFrame"]:Hide()
					else
						_G["InkedSV_ReagentContainerFrame"]:Show()
					end
				end
			end)
