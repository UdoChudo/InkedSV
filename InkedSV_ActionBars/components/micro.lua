...
local function RefreshMicrobar()
	if not InkedSV_MicroBar then return end 
	local lastParent = InkedSV_MicroBar;
	...
	InkedSV_MicroBar:SetSize(barWidth, buttonSize)
	for i=1,13 do
		...
		if(button) then
			...
			if lastParent == InkedSV_MicroBar then 
...
local InkedSVMicroButton_SetNormal = function()
...
local InkedSVMicroButtonsParent = function(self)
	if self ~= InkedSV_MicroBar then 
		self = InkedSV_MicroBar 
	end 
	for i=1,13 do
		local data = MOD.media.microMenuCoords[i]
		if(data) then
			local mButton = _G[data[1]]
			if(mButton) then mButton:SetParent(InkedSV_MicroBar) end
		end
	end 
end 
...
local MicroButton_OnEnter = function(self)
	if(self._fade) then
		InkedSV_MicroBar:FadeIn(0.2,InkedSV_MicroBar:GetAlpha(),1)
	end
...
local MicroButton_OnLeave = function(self)
	if(self._fade) then
		InkedSV_MicroBar:FadeOut(1,InkedSV_MicroBar:GetAlpha(),0)
	end
	if InCombatLockdown()then return end 
...
function MOD:UpdateMicroButtons()
	if(not SV.db.ActionBars.Micro.mouseover) then
		InkedSV_MicroBar:SetAlpha(1)
	else
		InkedSV_MicroBar:SetAlpha(0)
	end
...
function MOD:InitializeMicroBar()
	...
	local microBar = NewFrame('Frame', 'InkedSV_MicroBar', UIParent)
	...
	for i=1,16 do
		...
		if(data) then
			local button = _G[data[1]]
			if(button) then
				button:SetParent(InkedSV_MicroBar)
				button:SetSize(buttonSize, buttonSize + 28)
	...
	NewHook('MainMenuMicroButton_SetNormal', InkedSVMicroButton_SetNormal)
	MainMenuMicroButton:SetScript("OnUpdate",nil) -- Fix for problems with texture behind. Besides, No interest in the perf bar stuff as we already have the performance meters in InkedSV
	NewHook('UpdateMicroButtonsParent', InkedSVMicroButtonsParent)
	...
	InkedSVMicroButtonsParent(microBar)
	InkedSVMicroButton_SetNormal()
