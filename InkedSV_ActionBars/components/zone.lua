...
local SetButtonSpell = function(self, spellID, spellName, texture)
    ...
    local key = GetBindingKey('InkedSV_DRAENORZONE')
    if(key) then
...
if(not DraenorZoneAbilityFrame) then return end;
	local size = InkedSV_DraenorButtonHolder:GetHeight()
    local draenor = CreateFrame('Button', "InkedSV_DraenorZoneAbility", UIParent, 'SecureActionButtonTemplate, SecureHandlerStateTemplate, SecureHandlerAttributeTemplate');
    draenor:SetSize(size,size);
    draenor:SetPoint("CENTER", InkedSV_DraenorButtonHolder, "CENTER", 0, 0);
    draenor:SetAlpha(0);
