...
local Totems = CreateFrame("Frame", "InkedSV_TotemBar", UIParent);

function Totems:Refresh()
	for i = 1, MAX_TOTEMS do
		local slot = TOTEM_PRIORITIES[i]
		local haveTotem, name, start, duration, icon = GetTotemInfo(slot)
		local InkedSVtotem = _G["InkedSV_TotemBarTotem"..slot]
		if(haveTotem) then
			InkedSVtotem:Show()
			InkedSVtotem.Icon:SetTexture(icon)
			CooldownFrame_Set(InkedSVtotem.CD, start, duration, 1)
			local blizztotem = _G["TotemFrameTotem"..slot]
			local tslot = blizztotem.slot
			if(tslot and tslot > 0) then
				local anchor = _G["InkedSV_TotemBarTotem"..tslot]
...
function MOD:InitializeTotemBar()
	...
	for i = 1, MAX_TOTEMS do
		local slot = TOTEM_PRIORITIES[i]
		local totem = CreateFrame("Button", "InkedSV_TotemBarTotem"..slot, Totems)
		...
		totem.Icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
		totem.CD = CreateFrame("Cooldown", "InkedSV_TotemBarTotem"..slot.."Cooldown", totem, "CooldownFrameTemplate")
