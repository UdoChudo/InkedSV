...
local DEFAULT_MAIN_ANCHOR = _G["InkedSV_DockBottomCenter"];
MOD.ButtonCache = {};
MOD.MainAnchor = CreateFrame("Frame", "InkedSV_ActionBarMainAnchor");
...
local function ModifyActionButton(button, noStyle)
	...
	if SV.db.ActionBars.hotkeytext then
		hotkey:ClearAllPoints()
		hotkey:SetAllPoints()
		hotkey:SetFontObject(InkedSV_Font_Default)
...
function MOD:UpdateBarBindings(pet, stance)
	if stance == true then
		local bar = _G["InkedSV_StanceBar"]
		local bindText = bar.binding

	  	for i=1,NUM_STANCE_SLOTS do
	  		local name = ("InkedSV_StanceBarButton%s"):format(i)
	  		local hkname = ("InkedSV_StanceBarButton%sHotKey"):format(i)
  	...
  	if pet == true then
  		local bar = _G["InkedSV_PetActionBar"]
		local bindText = bar.binding
...
function MOD:UpdateAllBindings(event)
	...
	for i = 1, TOTAL_BARS do
		local barName = ("InkedSV_ActionBar%d"):format(i)
...
function MOD:UpdateBarPagingDefaults()
	...
	for i=2, TOTAL_BARS do
		...
		local bar = _G["InkedSV_Action" .. id];
		if(bar) then
			local parse = '';
	...
	local mainbar = _G["InkedSV_ActionBar1"];
	if(mainbar) then
...
function MOD:RefreshMainAnchor()
	...
	for i = 1, TOTAL_BARS do
		id = ("Bar%d"):format(i);
		bar = _G[("InkedSV_Action%s"):format(id)];
		if((i < 3) or (i > 5)) then
...
do
	...
	function MOD:RefreshBar(id)
		...
		if(id == "Pet") then
			bar = _G["InkedSV_PetActionBar"]
			isPet = true
		elseif(id == "Stance") then
			bar = _G["InkedSV_StanceBar"]
			isStance = true
		else
			bar = _G[("InkedSV_Action%s"):format(id)]
		end
		...
		if(isPet or isStance) then
			...
			local moverName = ("InkedSV_Action%d_MOVE"):format(id);
	...
	if(bar and bar.buttons) then
		...
		if(enabled) then
			...
			for k = 1, #bar.buttons do
				...
				local clickBind = ("InkedSV_ActionBar1Button%d"):format(k);
...
local function SetStanceBarButtons()
	...
	for i = 1, maxButtons do
		local button = _G["InkedSV_StanceBarButton"..i]
		local icon = _G["InkedSV_StanceBarButton"..i.."Icon"]
		if i <= maxForms then
...
function MOD:UpdateUniqueBars()
	local bar = MOD.AltVehicleBar or _G["InkedSV_ActionBar1"];
...
local InkedSVOptionsPanel_OnEvent = function()
...
local InkedSVButton_ShowOverlayGlow = function(self)
...
local ResetAllBindings = function(self)
	...
	local bar
	for i = 1, TOTAL_BARS do
		bar = _G[("InkedSV_ActionBar%d"):format(i)]
		if(bar) then
			ClearOverrideBindings(bar)
		end
	end

	ClearOverrideBindings(_G["InkedSV_PetActionBar"])
	ClearOverrideBindings(_G["InkedSV_StanceBar"])
...
local barBindingIndex = {
	...
	"InkedSVACTIONBAR6BUTTON%d",
	"InkedSVACTIONBAR7BUTTON%d",
	"InkedSVACTIONBAR8BUTTON%d",
	"InkedSVACTIONBAR9BUTTON%d",
	"InkedSVACTIONBAR10BUTTON%d"
}
...
CreateActionBars = function(self)
	for i = 1, TOTAL_BARS do
		...
		local barName = ("InkedSV_Action%s"):format(barID)
		...
		if(i == 3) then
			thisBar:SetPoint("BOTTOMLEFT", _G["InkedSV_ActionBar1"], "BOTTOMRIGHT", space, 0)
		elseif(i == 4) then
			thisBar:SetPoint("RIGHT", SV.Screen, "RIGHT", -space, 0)
		elseif(i == 5) then
			thisBar:SetPoint("BOTTOMRIGHT", _G["InkedSV_ActionBar1"], "BOTTOMLEFT", -space, 0)
		else
...
do
	local function UpdateShapeshiftForms(self, event)
	  if InCombatLockdown() or not _G["InkedSV_StanceBar"] then return end

	  local stanceBar = _G["InkedSV_StanceBar"];
	  ...
	  for i = 1, NUM_STANCE_SLOTS do
		if(not stanceBar.buttons[i]) then
		  stanceBar.buttons[i] = CreateFrame("CheckButton", format("InkedSV_StanceBarButton%d", i), stanceBar, "StanceButtonTemplate")
	...
	local function UpdateShapeshiftCD()
	  ...
	  for i = 1, NUM_STANCE_SLOTS do
		if i  <= maxForms then
		  local cooldown = _G["InkedSV_StanceBarButton"..i.."Cooldown"]
	...
	CreateStanceBar = function(self)
	  local barID = "Stance";
	  local maxForms = GetNumShapeshiftForms();
	  local stanceBar = NewActionBar("InkedSV_StanceBar")
	  stanceBar.binding = "CLICK InkedSV_StanceBarButton%d:LeftButton"
	  ...
	  for i = 1, NUM_STANCE_SLOTS do
		stanceBar.buttons[i] = _G["InkedSV_StanceBarButton"..i]
	  end
...
do
	...
	local RefreshPet = function(self, event, unit)
		...
		local actionName, actionIcon, isToken, isActive, autoCastAllowed, autoCastEnabled, spellID  = GetPetActionInfo(i)
	...
	CreatePetBar = function(self)
		local barID = "Pet";
		local petBar = NewActionBar("InkedSV_PetActionBar")
...
local CreateExtraBar = function(self)
	local specialBar = CreateFrame("Frame", "InkedSV_SpecialAbility", SV.Screen)
...
local function pushSpellToActionBar(self,spellID,slotIndex,slotPos)
	local slotParent = _G["InkedSV_Bar1Button"..slotPos];
...
function MOD:Load()
	...
	NewHook("BlizzardOptionsPanel_OnEvent", InkedSVOptionsPanel_OnEvent)
	NewHook("ActionButton_ShowOverlayGlow", InkedSVButton_ShowOverlayGlow)
