...
local DriverFrame = CreateFrame('Frame', 'InkedSV_Nameplates_DriverFrame', UIParent)
local UnitFrameMixin = {}
local UnitBuffMixin = {}

local path= "Interface\\Addons\\InkedSV_NamePlates\\assets\\"
...
local scanner = CreateFrame("GameTooltip", "InkedSV_NameplatesScanner", nil, "GameTooltipTemplate")
local questtipLine = setmetatable({}, { __index = function(k, i)
	local line = _G["InkedSV_NameplatesScannerTextLeft" .. i]
...
function UnitFrameMixin:Create(unitframe)
	...
	self.level = h:CreateFontString(nil, 'OVERLAY',"InkedSV_Font_NamePlate_Number")
	...
	do
		...
		c.Text = c:CreateFontString(nil, 'OVERLAY', "InkedSV_Font_NamePlate")
	...
	self.name = h:CreateFontString(nil, 'ARTWORK', "InkedSV_Font_NamePlate")
	...
	self.questText = self:CreateFontString(nil, nil, "InkedSV_Font_NamePlate_Number")
