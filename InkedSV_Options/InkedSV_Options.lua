...
local AceInkedWidgets = AceInkedWidgets;
...
InkedSVOptions.FilterOptionGroups = {};
InkedSVOptions.FilterOptionSpells = {};
...
local function RefreshProfileOptions()
	...
	local currentProfile = InkedSVLib:CurrentProfile()
	...
	optionGroup.common = {
		...
		args = {
			...
			dualSpec = {
				...
				get = function() return InkedSVLib:CheckDualProfile() end,
				set = function(key, value) InkedSVLib:ToggleDualProfile(value) end
			},
			mpEnable = {
				order = 4,
				type = "toggle",
				...
				set = function(key, value)
					if(not value) then
						InkedSVLib:SetMasterProfile()
					else
						InkedSVLib:SetMasterProfile(currentProfile)
					end
					SV:SavedPopup()
			...
			mpSelect = {
				...
				get = function() return InkedSVLib:CheckMasterProfile() end,
				set = function(key, value) InkedSVLib:SetMasterProfile(value) SV:SavedPopup() RefreshProfileOptions() end,
				values = InkedSVLib:GetProfiles(),
				disabled = function()
					local t = InkedSVLib:CheckMasterProfile()
					return (not t)
				end,
			},
			profileInterface = {
	...
	optionGroup.actions = {
		...
		args = {
			importdesc = {
				...
				type = "description",
				name = function()
					if(InkedSVLib:CheckDualProfile()) then
			...
			export = {
				...
				type = "input",
				get = false,
				set = function(key, value) InkedSVLib:CloneDatabase(value) SV:SavedPopup() RefreshProfileOptions() end,
				disabled = function()
					local t = InkedSVLib:CheckProfiles()
					return ((not t) or InkedSVLib:CheckDualProfile())
				end,
			},
			copy = {
				order = 4,
	...
	optionGroup.removal = {
		...
		args = {
			...
			delete = {
				...
				set = function(key, value) InkedSVLib:Remove(value) end,
				values = InkedSVLib:GetProfiles(),
				disabled = function() local t = InkedSVLib:CheckProfiles() return (not t) end,
				confirm = true,
...
function InkedSVOptions:SetToFontConfig(font)
...
function InkedSVOptions:SetFilterOptions(filterType, selectedSpell)
	local FILTER
...
function InkedSVOptions:SetToFilterConfig(newFilter)
...
SV.Options.args.primary = {
	...
	args = {
		introGroup1 = {
			...
			type = "description",
			width = "full",
			image = function() return SV.SplashImage, 256, 256 end,
		},
		introGroup2 = {
			order = 2,
		...
		colors = {
			...
			args = {
				customClassColor = {
					...
					desc = L["Use the enhanced class colors provided by InkedSV"],
		...
		common = {
			...
			args = {
				threatbar = {
					...
					desc = L["Enable/disable the custom InkedSV threat meter"],
...
InkedSVLib:LoadModuleOptions()
RefreshProfileOptions()

InkedSVOptions.FilterOptionGroups['_NEW'] = function(filterType)
	return function()
		local RESULT, FILTER
		if(SV.db.Filters.Custom[filterType]) then
			FILTER = SV.db.Filters.Custom[filterType]
					...
					addSpell = {
						...
						set = function(key, value)
							...
							elseif(not FILTER[value]) then
								...
								InkedSVOptions:SetFilterOptions(filterType)
								SV.Events:Trigger("AURA_FILTER_OPTIONS_CHANGED");
							end
						end
					},
...
InkedSVOptions.FilterOptionSpells['_NEW'] = function(filterType)
	return function()
		local RESULT, FILTER
		if(SV.db.Filters.Custom[filterType]) then
			FILTER = SV.db.Filters.Custom[filterType]
...
SV.Options.args.Filters.args.createFilter = {
	...
	set = function(key, value)
		...
		elseif(SV.db.Filters.Custom[value]) then
			...
			InkedSVOptions:SetFilterOptions(value);
		end
	end
};
...
SV.Options.args.Filters.args.selectFilter = {
	...
	set = function(key, value) InkedSVOptions:SetFilterOptions(value) end,
