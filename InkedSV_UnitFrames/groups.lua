...
local TankUnitUpdate = function(self)
    ...
    do
        local nametext = self.TextGrip.Name;
        if oUF_InkedSV.colors.healthclass then
...
function MOD:SetGroupHeader(parentFrame, filter, layout, headerName, token, groupTag)
    ...
    if(token == "raidpet") then
        template1 = "InkedSV_UNITPET"
        template2 = "SecureGroupPetHeaderTemplate"
    elseif(token == "party") then
        template1 = "InkedSV_UNITPET, InkedSV_UNITTARGET"
    elseif(token == "tank") then
        filter = "MAINTANK"
        template1 = "InkedSV_UNITTARGET"
    ...
    local groupHeader = oUF_InkedSV:SpawnHeader(headerName, template2, nil,
...
function MOD:GetGroupFrame(token, layout)
    if(not self.Headers[token]) then
        oUF_InkedSV:RegisterStyle(layout, BuildTemplates[token])
        oUF_InkedSV:SetActiveStyle(layout)
        local groupFrame = CreateFrame("Frame", layout, _G.InkedSV_UnitFrameParent, "SecureHandlerStateTemplate")
...
function MOD:SetCustomFrame(token, layout)
    if(not self.Headers[token]) then
        oUF_InkedSV:RegisterStyle(layout, BuildTemplates[token])
        oUF_InkedSV:SetActiveStyle(layout)
        local groupFrame = self:SetGroupHeader(_G.InkedSV_UnitFrameParent, nil, layout, layout, token)
        self.Headers[token] = groupFrame
    end
	print(self.Headers[token]:GetAttribute("groupFilter"))
...
function MOD:SetGroupFrame(token, forceUpdate)
    ...
    local layout = "InkedSV_"..realName
