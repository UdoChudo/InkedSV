...
local FontMapping = {
	["player"] = "InkedSV_Font_Unit",
	["target"] = "InkedSV_Font_Unit",
	["targettarget"] = "InkedSV_Font_Unit_Small",
	["pet"] = "InkedSV_Font_Unit",
	["pettarget"] = "InkedSV_Font_Unit_Small",
	["focus"] = "InkedSV_Font_Unit",
	["focustarget"] = "InkedSV_Font_Unit_Small",
	["boss"] = "InkedSV_Font_Unit",
	["arena"] = "InkedSV_Font_Unit",
	["party"] = "InkedSV_Font_Unit_Small",
	["raid"] = "InkedSV_Font_Unit_Small",
	["raidpet"] = "InkedSV_Font_Unit_Small",
	["tank"] = "InkedSV_Font_Unit_Small",
	["assist"] = "InkedSV_Font_Unit_Small",
};
local ThreatMapping = {
	["player"] = true,
	["pet"] = true,
...
local ELITE_TOP = [[Interface\Addons\InkedSV_UnitFrames\assets\Border\ELITE-TOP]];
local ELITE_BOTTOM = [[Interface\Addons\InkedSV_UnitFrames\assets\Border\ELITE-BOTTOM]];
local ELITE_RIGHT = [[Interface\Addons\InkedSV_UnitFrames\assets\Border\ELITE-RIGHT]];
local STUNNED_ANIM = [[Interface\Addons\InkedSV_UnitFrames\assets\UNIT-STUNNED]];
local AGGRO_TEXTURE = [[Interface\AddOns\InkedSV_UnitFrames\assets\UNIT-AGGRO]];
...
local function CreateThreat(frame, unit)
	...
	if(unit == "player") then
		local aggro = CreateFrame("Frame", "InkedSV_PlayerThreatAlert", frame)
...
function MOD:SetActionPanel(frame, unit, noHealthText, noPowerText, noMiscText)
	...
	if(unit and (unit == "player" or unit == "pet" or unit == "target" or unit == "targettarget" or unit == "focus" or unit == "focustarget")) then
		...
		frame.StatusPanel.media = {
			[[Interface\Addons\InkedSV_UnitFrames\assets\TARGET-DC]],
			[[Interface\Addons\InkedSV_UnitFrames\assets\TARGET-DEAD]],
			[[Interface\Addons\InkedSV_UnitFrames\assets\TARGET-TAPPED]]
		}
	else
		frame.StatusPanel:SetSize(50, 50)
		frame.StatusPanel:SetPoint("CENTER", frame, "CENTER", 0, 0)
		frame.StatusPanel.media = {
			[[Interface\Addons\InkedSV_UnitFrames\assets\UNIT-DC]],
			[[Interface\Addons\InkedSV_UnitFrames\assets\UNIT-DEAD]],
			[[Interface\Addons\InkedSV_UnitFrames\assets\UNIT-TAPPED]]
		}
	end
...
local OverlayHealthUpdate = function(health, unit)
	...
	elseif(health.colorOverlay) then
		local t = oUF_InkedSV.colors.health
...
function MOD:CreateHealthBar(frame, hasbg)
	...
	flasher[1]:SetTexture([[Interface\Addons\InkedSV_UnitFrames\assets\UNIT-HEALTH-ANIMATION]])
...
function MOD:CreateAltPowerBar(frame)
	...
	altPower.text:SetFontObject(InkedSV_Font_Unit)
...
function MOD:PostUpdatePower(unit, value, max)
	...
	local colors = oUF_InkedSV.colors.power[powerToken]
	...
	if isPlayer and self.colorClass then
		...
		colors = oUF_InkedSV["colors"].class[class]
