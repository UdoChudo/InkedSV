...
local AFFLICTED_SKIN = [[Interface\AddOns\InkedSV_UnitFrames\assets\UNIT-AFFLICTED]];
...
function MOD:CreateRaidDebuffs(frame)
	...
	raidDebuff.icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
	raidDebuff.icon:InsetPoints(raidDebuff)
	raidDebuff.count = raidDebuff:CreateFontString(nil, "OVERLAY")
	raidDebuff.count:SetFontObject(InkedSV_Font_UnitAura)
	...
	raidDebuff.time:SetFontObject(InkedSV_Font_UnitAura)
...
function MOD:CreateGladiator(frame)
	...
	trinket.Icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))

	trinket.Unavailable = trinket:CreateTexture(nil, "OVERLAY")
	trinket.Unavailable:SetAllPoints(trinket)
	trinket.Unavailable:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
	...
	badge.Icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
...
function MOD:CreateExperienceRepBar(frame)
	...
	if db.playerExpBar then
		...
		xp.Value:SetFontObject(InkedSV_Font_Default)
	...
	if db.playerRepBar then
		...
		rep.Value:SetFontObject(InkedSV_Font_Default)
...
-- 	bar.text:SetFontObject(InkedSV_Font_Pixel)
