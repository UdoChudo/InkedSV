...
local UpdatePlayerFrame = function(self)
    ...
    self.colors = oUF_InkedSV.colors;
...
CONSTRUCTORS["target"] = function(self, unit)
   ...
   -- self.RaidRoleIndicator = MOD:CreateRaidIcon(self)
    ...
    local xray = CreateFrame("Button", "InkedSV_XRayFocus", self, "SecureActionButtonTemplate")
    ...
    xray.icon:SetTexture([[Interface\Addons\InkedSV\assets\textures\Doodads\UNIT-XRAY]])
...
CONSTRUCTORS["targettarget"] = function(self, unit)
  ...
  --  self.RaidRoleIndicator = MOD:CreateRaidIcon(self)
    self.Range = { insideAlpha = 1, outsideAlpha = 1 }
    self:SetPoint("LEFT", InkedSV_Target, "RIGHT", 4, 0)
...
local UpdatePetFrame = function(self)
    ...
    do
        if InkedSV_Player and not InCombatLockdown()then
            self:SetParent(InkedSV_Player)
        end
    end
...
CONSTRUCTORS["pet"] = function(self, unit)
    ...
    --self.RaidRoleIndicator = MOD:CreateRaidIcon(self)
    self.Range = { insideAlpha = 1, outsideAlpha = 1 }
    self:SetPoint("RIGHT", InkedSV_Player, "LEFT", -4, 0)
...
local UpdatePetTargetFrame = function(self)
    ...
    do
        if InkedSV_Pet and not InCombatLockdown()then
            self:SetParent(InkedSV_Pet)
        end
    end
...
CONSTRUCTORS["pettarget"] = function(self, unit)
    ...
    MOD:CreateAuraFrames(self, key)
    self.Range = { insideAlpha = 1, outsideAlpha = 1 }
    self:SetPoint("BOTTOM", InkedSV_Pet, "TOP", 0, 7)
    self.snapOffset = -7
...
CONSTRUCTORS["focus"] = function(self, unit)
   ...
   -- self.RaidRoleIndicator = MOD:CreateRaidIcon(self)
    ...
    local xray = CreateFrame("Button", "InkedSV_XRayFocusClear", self, "SecureActionButtonTemplate")
    ...
    xray:SetFrameStrata("MEDIUM")
    xray.icon = xray:CreateTexture(nil,"ARTWORK")
    xray.icon:SetTexture([[Interface\Addons\InkedSV\assets\textures\Doodads\UNIT-XRAY-CLOSE]])
    xray.icon:SetAllPoints(xray)
    xray.icon:SetAlpha(0)
    ...
    self:SetPoint("BOTTOMRIGHT", InkedSV_Target, "TOPRIGHT", 0, 220)
...
CONSTRUCTORS["focustarget"] = function(self, unit)
   ...
   -- self.RaidRoleIndicator = MOD:CreateRaidIcon(self)
    self.Range = { insideAlpha = 1, outsideAlpha = 1 }
    self:SetPoint("LEFT", InkedSV_Focus, "RIGHT", 12, 0)
    self.snapOffset = -7
...
local UpdateBossFrame = function(self)
    ...
    local holder = _G['InkedSV_Boss1_MOVE']
    local UNIT_WIDTH = db.width;
    local UNIT_HEIGHT = db.height;
...
local UpdateArenaFrame = function(self)
    ...
    local INDEX = self:GetID() or 1;
    local holder = _G['InkedSV_Arena1_MOVE'];
    local UNIT_WIDTH = db.width;
    local UNIT_HEIGHT = db.height
...
CONSTRUCTORS["arena"] = function(self, unit)
    ...
    if(not _G[prepName]) then
        ...
        icon.Icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
...
local ArenaPrepHandler_OnEvent = function(self, event)
    ...
    if event == "PLAYER_LOGIN" then
        for i = 1, 5 do
            prepframe = _G["InkedSV_Arena"..i.."PrepFrame"]
            if(prepframe) then
                prepframe:SetAllPoints(_G["InkedSV_Arena"..i])
            end
        end
...
function MOD:SetUnitFrame(key)
    ...
    local styleName = "InkedSV_"..realName
    local frame
    if not self.Units[unit] then
        oUF_InkedSV:RegisterStyle(styleName, CONSTRUCTORS[key])
        oUF_InkedSV:SetActiveStyle(styleName)
        frame = oUF_InkedSV:Spawn(unit, styleName)
        self.Units[unit] = frame
    else
        frame = self.Units[unit]
    end
    if frame:GetParent() ~= InkedSV_UnitFrameParent then
        frame:SetParent(InkedSV_UnitFrameParent)
    end
