...
do
	...
	function oUF_InkedSV:DisableBlizzard(unit)
...
function MOD:RefreshUnitColors()
	if(not SV.db.general.customClassColor) then
		for eclass, color in next, RAID_CLASS_COLORS do
			oUF_InkedSV.colors.class[eclass] = {color.r, color.g, color.b}
		end
	else
	...
	oUF_InkedSV.colors.smooth = {1, 0, 0, 1, 1, 0, r, g, b}
...
function MOD:RefreshUnitFrames()
	...
	if SV.db.UnitFrames.disableBlizzard then
		oUF_InkedSV:DisableBlizzard('party')
	end
    ...
    if(unitDB and unitDB.enable) then
        ...
        if(self.Castbar and (unitDB.castbar)) then
            if(unitDB.castbar.useCustomColor) then
				...
				self.Castbar.CastColor = oUF_InkedSV.colors.casting
				self.Castbar.SparkColor = oUF_InkedSV.colors.spark
			end
        end
    end
end
...
function MOD:ADDON_LOADED(event, addon)
	...
	if addon == 'Blizzard_ArenaUI' then
		oUF_InkedSV:DisableBlizzard('arena')
...
function MOD:Load()
	...
	local InkedSV_UnitFrameParent = CreateFrame("Frame", "InkedSV_UnitFrameParent", SV.Screen, "SecureHandlerStateTemplate")
	RegisterStateDriver(InkedSV_UnitFrameParent, "visibility", "[petbattle] hide; show")
	...
	if(SV.db.UnitFrames.disableBlizzard) then
		...
		oUF_InkedSV:DisableBlizzard("player")
		oUF_InkedSV:DisableBlizzard("pet")
		oUF_InkedSV:DisableBlizzard("pettarget")
		oUF_InkedSV:DisableBlizzard("target")
		oUF_InkedSV:DisableBlizzard("targettarget")
		oUF_InkedSV:DisableBlizzard("focus")
		oUF_InkedSV:DisableBlizzard("focustarget")
		oUF_InkedSV:DisableBlizzard("boss")
