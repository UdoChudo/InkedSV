...
local function CollectionsJournalStyle()
	...
	for i = 1, #buttons do
		...
		if(button) then
			...
			local bar = _G["InkedSV_MountSelectBar"..i]
	...
	PetJournalPetCardPetInfoIcon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
	...
	for b = 1, 6 do 
		...
		S.icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
	...
	MountJournal.SummonRandomFavoriteButton.texture:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
