...
local PetBattleActionBar = CreateFrame("Frame", "InkedSV_PetBattleActionBar", UIParent)
...
local _hook_UpdateDisplay = function(self)
	self.Icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
...
local function PetBattleStyle()
	...
	local InkedSV_DockBottomCenter = _G.InkedSV_DockBottomCenter;
	if(InkedSV_DockBottomCenter) then
		PetBattleActionBar:SetPoint("BOTTOM", InkedSV_DockBottomCenter, "TOP", 0, 4)
	else
