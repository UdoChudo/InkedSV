...
local ToastMinion = CreateFrame("Frame", "InkedSV_ToastMinion");
local ToastVault;
local anchor = "";
local addonLoaded;
...
local function InkedSV_SocialQueueUtil_GetQueueName(queue, nameFormatter)
	local nameText = {};
			...
			if (getQueues[1].queueData.queueType == "lfglist") then
				...
				for id, queue in pairs(getQueues) do
					if (queue.eligible) then
						eQueue = true;
						local queueTable = InkedSV_SocialQueueUtil_GetQueueName(queue.queueData);
						local queueName = "";
