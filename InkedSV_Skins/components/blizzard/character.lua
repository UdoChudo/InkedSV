	...
	for _,slotName in pairs(CharacterSlotNames) do
		...
		if(charSlot) then
			if(not charSlot.Panel) then
				...
				if(iconTex) then
					iconTex:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
...
local function PaperDollEquipmentManagerPane_OnShow()
	for i,btn in pairs(PaperDollEquipmentManagerPane.buttons) do
		if(btn) then
			...
			btn.icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
		...
		for i = 1, GetCurrencyListSize() do
			...
			if(currency) then
				...
				if currency.icon then
					 currency.icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
				end
			end
		end
