...
local RING_TEXTURE = [[Interface\AddOns\InkedSV_Skins\artwork\FOLLOWER-RING]]
local LVL_TEXTURE = [[Interface\AddOns\InkedSV_Skins\artwork\FOLLOWER-LEVEL]]
...
local function StyleTextureIcon(frame)
	...
	frame.Texture:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
...
local function StyleIconElement(frame)
	...
	if(frame.Icon) then
		...
		if(frame.Quantity) then
	    	frame.Quantity:SetFontObject(InkedSV_Font_Number)
...
local function _hook_GarrisonMissionFrame_SetItemRewardDetails(item)
    ...
    if(item.Icon) then
		...
		item.Icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
    ...
    if(item.Quantity) then
    	item.Quantity:SetFontObject(InkedSV_Font_Number)
    ...
    for i = 1, #reagents do
    	if(reagents[i] and (not reagents[i].Panel)) then
        	...
        	if(reagents[i].Icon) then
				reagents[i].Icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
			end
		end
    end
end
...
local function LoadGarrisonStyle()
  ...
  hooksecurefunc("GarrisonBuildingInfoBox_ShowFollowerPortrait", _hook_GarrisonBuildingInfoBox_ShowFollowerPortrait)
	...
	missionChance:SetFontObject(InkedSV_Font_Number_Huge)
	local chanceLabel = GarrisonMissionFrame.MissionTab.MissionPage.RewardsFrame.ChanceLabel
	chanceLabel:SetFontObject(InkedSV_Font_Header)
	...
	GarrisonCapacitiveDisplayFrame.CapacitiveDisplay.ShipmentIconFrame.Icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
