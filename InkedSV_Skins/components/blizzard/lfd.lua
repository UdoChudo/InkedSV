  ...
  for u = 1, LFD_MAX_REWARDS do
    ...
    if t then
      if not t.restyled then
        ...
        local z = _G["ScenarioQueueFrameRandomScrollFrameChildFrameItem"..u.."NameFrame"]icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
        icon:SetDrawLayer("OVERLAY")
        y:SetDrawLayer("OVERLAY")
        z:SetTexture()
        z:SetSize(118, 39)
...
local function LFDFrameStyle()
    ...
    if(frame) then
      ...
      if(frame.cover) then
        frame.cover:SetTexture("Interface\\AddOns\\InkedSV_Skins\\artwork\\UI-LFG-ICON-ROLES")
      end
      frame:SetNormalTexture("Interface\\AddOns\\InkedSV_Skins\\artwork\\UI-LFG-ICON-ROLES")
    end
    end
    ...
    if(frame) then
      ...
      if(tex) then
        tex:SetTexture("Interface\\AddOns\\InkedSV_Skins\\artwork\\UI-LFG-ICON-ROLES")
      end
    end
    end
    ...
    for i=1, #QueueStatusFrame.StatusEntriesPool do
      ...
      if(node.RoleIcon1) then
        node.RoleIcon1:SetTexture("Interface\\AddOns\\InkedSV_Skins\\artwork\\UI-LFG-ICON-ROLES")
      end
      if(node.RoleIcon2) then
        node.RoleIcon2:SetTexture("Interface\\AddOns\\InkedSV_Skins\\artwork\\UI-LFG-ICON-ROLES")
      end
      if(node.RoleIcon3) then
        node.RoleIcon3:SetTexture("Interface\\AddOns\\InkedSV_Skins\\artwork\\UI-LFG-ICON-ROLES")
      end
      if(node.HealersFound) then
        node.HealersFound.Cover:SetTexture("Interface\\AddOns\\InkedSV_Skins\\artwork\\UI-LFG-ICON-ROLES")
        node.HealersFound.Texture:SetTexture("Interface\\AddOns\\InkedSV_Skins\\artwork\\UI-LFG-ICON-ROLES")
      end
      if(node.TanksFound) then
        node.TanksFound.Cover:SetTexture("Interface\\AddOns\\InkedSV_Skins\\artwork\\UI-LFG-ICON-ROLES")
        node.TanksFound.Texture:SetTexture("Interface\\AddOns\\InkedSV_Skins\\artwork\\UI-LFG-ICON-ROLES")
      end
      if(node.DamagersFound) then
        node.DamagersFound.Cover:SetTexture("Interface\\AddOns\\InkedSV_Skins\\artwork\\UI-LFG-ICON-ROLES")
        node.DamagersFound.Texture:SetTexture("Interface\\AddOns\\InkedSV_Skins\\artwork\\UI-LFG-ICON-ROLES")
      end
    end
    end
    ...
    if(button) then
      ...
      button.icon:SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
    ...
    if not LFRQueueFrameSpecificListScrollFrame.styled then
      ...
      for u = 1, 2 do
        ...
        C:GetNormalTexture():SetTexCoord(unpack(_G.InkedSV_ICON_COORDS))
