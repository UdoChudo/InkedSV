...
function MOD:ADDON_LOADED(event, addon)
	...
	-- if(addon == 'InkedSV_!Options') then
	-- 	self:Style(addon, MOD.StyleInkedSVOptions)
	-- end
...
function MOD:Load()
	...
	self.Docklet = SV.Dock:NewDocklet("BottomRight", "InkedSV_SkinsDock", "Addon Docklet", [[Interface\AddOns\InkedSV_Skins\artwork\DOCK-ICON-ADDON]]);
	...
	self.Docklet.Dock1 = CreateFrame("Frame", "InkedSV_SkinsDockAddon1", self.Docklet);
	...
	self.Docklet.Dock2 = CreateFrame("Frame", "InkedSV_SkinsDockAddon2", self.Docklet);
	...
	SV:AddSlashCommand("skinned", "List all addons skinned by InkedSV", ListSkinnedAddons);
